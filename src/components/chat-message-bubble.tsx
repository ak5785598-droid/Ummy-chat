...
export const ChatMessageBubble = memo(({ bubbleId, isMe, children, className }: ChatMessageBubbleProps) => {
  if (!bubbleId || bubbleId === 'None') {
    return (
      <div className={cn(
        "px-3 py-1.5 rounded-2xl text-[13px] font-medium shadow-sm border max-w-full min-w-[50px] relative will-change-transform",
        isMe ? "bg-cyan-500/30 text-cyan-200 border-cyan-500/40 shadow-[0_0_15px_rgba(6,182,212,0.1)]" : "bg-white/10 text-white border-white/5",
        className
      )}>
        {children}
      </div>
  ...
  const styles: Record<string, { bg: string, text: string, border: string, shadow: string, decorator?: React.ReactNode, tailColor: string }> = {
    ...
    'evil-bubble': {
      ...
      decorator: (
        <>
          <div className="absolute -top-3 left-3 text-xl drop-shadow-[0_0_8px_rgba(168,85,247,0.8)] outline-none">😈</div>
    ...
    'candy-bubble': {
      ...
      shadow: 'shadow-[0_2px_10px_rgba(255,154,154,0.4)]',
    ...
    'neon-cyber': {
      bg: 'bg-black/85', // Removed backdrop-blur-xl
      text: 'text-[#00ffff] font-mono font-bold tracking-wider',
      border: 'border-[#00ffff] border-[1.5px]',
  ...
  return (
    <div className={cn(
      "relative px-4 py-2 rounded-2xl max-w-full transition-all flex items-center min-h-[38px] min-w-[60px] will-change-transform",
      config.bg,
      config.text,
      config.border,
      config.shadow,
