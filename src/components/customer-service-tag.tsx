...
/**
 * High-Fidelity Customer Service Tag Component.
 * Designed to mirror the reference image style:
 * - Glossy blue capsule background.
...
export function CustomerServiceTag({ className, size = 'md' }: { className?: string, size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={cn("relative inline-flex items-center justify-center select-none group", className)} style={{ transform: `scale(${scale})`, transformOrigin: 'left center' }}>
      {/* Outer Ambient Glow */}
      <div className="absolute inset-0 bg-blue-400/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
      
      {/* Main Tag Container */}
      <div className={cn(
        "relative flex items-center gap-2 pl-1 pr-4 py-1.5 bg-gradient-to-b from-[#38bdf8] via-[#0284c7] to-[#0369a1] rounded-full border-[3px] border-[#fbbf24] shadow-[0_0_15px_rgba(251,191,36,0.6),inset_0_2px_4px_rgba(255,255,255,0.3)] overflow-hidden",
        ...
        <span className="relative z-30 font-headline text-[13px] font-black text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.5)] tracking-tight">
          Customer Service
        </span>
        ...
        <div className="absolute inset-0 pointer-events-none z-40">
           ...
           <div className="absolute bottom-1 left-10 w-0.5 h-0.5 bg-blue-200 rounded-full animate-pulse" />
