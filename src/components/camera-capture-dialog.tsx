...
export function CameraCaptureDialog({ open, onOpenChange, onCapture, title = "Capture Vibe" }: CameraCaptureDialogProps) {
 ...
 return (
  <Dialog open={open} onOpenChange={handleClose}>
   <DialogContent className="sm:max-w-[425px] rounded-t-[2.5rem] md:rounded-3xl bg-white text-black p-0 overflow-hidden font-sans border-none shadow-2xl">
    <DialogHeader className="p-6 border-b border-gray-50">
     <DialogTitle className="text-2xl font-bold uppercase tracking-tight text-center">{title}</DialogTitle>
    </DialogHeader>
    
    <div className="p-6 flex flex-col items-center gap-4">
     <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-black shadow-inner border-4 border-gray-100">
      <video 
       ref={videoRef} 
      ...
      {hasCameraPermission === false && (
       <div className="absolute inset-0 flex items-center justify-center p-6 text-center bg-gray-900">
        <Alert variant="destructive" className="border-none bg-red-500/10 text-red-500">
         <AlertTitle className="font-bold uppercase ">Access Required</AlertTitle>
    ...
    <DialogFooter className="p-6 pt-0 flex gap-3">
     {capturedImage ? (
      <>
       <Button variant="outline" onClick={retake} className="flex-1 h-14 rounded-2xl font-bold uppercase text-xs border-2">
        <RefreshCcw className="mr-2 h-4 w-4" /> Retake
       </Button>
       <Button onClick={handleConfirm} className="flex-1 h-14 rounded-2xl font-bold uppercase text-xs bg-green-500 hover:bg-green-600 text-white shadow-xl shadow-green-500/20">
        <Check className="mr-2 h-4 w-4" /> Confirm
       </Button>
      </>
     ) : (
      <>
       <Button variant="outline" onClick={handleClose} className="flex-1 h-14 rounded-2xl font-bold uppercase text-xs border-2">
        Cancel
       </Button>
       <Button 
