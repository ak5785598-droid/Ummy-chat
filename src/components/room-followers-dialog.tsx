...
const RichLevelBadge = ({ level }: { level: number }) => (
  <div className="flex items-center gap-1 bg-gradient-to-r from-blue-400 via-cyan-500 to-blue-400 px-2 py-0.5 rounded-full border border-white/30 shadow-sm relative overflow-hidden shrink-0">
    ...
    <span className="text-[8px] font-black text-white">Lv.{level}</span>
  </div>
);
...
const GenderCircle = ({ gender }: { gender: string | null | undefined }) => (
  <div className={cn(
    "h-3.5 w-3.5 rounded-full flex items-center justify-center text-[7px] font-black text-white shrink-0 shadow-sm",
    ...
    <div className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
           <h4 className="font-black text-sm text-gray-900 truncate uppercase italic tracking-tight">{profile.username}</h4>
           {showBadges && (
...
export function RoomFollowersDialog({ open, onOpenChange, room }: RoomFollowersDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px] h-[80vh] md:h-[600px] p-0 border-none bg-white text-black rounded-t-[3rem] md:rounded-[2.5rem] flex flex-col font-headline shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full duration-500">
        <DialogHeader className="p-6 pt-10 border-b border-gray-50 shrink-0">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <DialogTitle className="text-2xl font-black uppercase italic tracking-tighter">Followers</DialogTitle>
                 <DialogDescription className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tribal Hierarchy & Fans</DialogDescription>
              </div>
           </div>
        </DialogHeader>
        ...
        <ScrollArea className="flex-1">
           <div className="pb-10">
              ...
              <div className="px-4 py-2 bg-yellow-50/30 border-b border-yellow-100/50">
                 <span className="text-[9px] font-black uppercase tracking-widest text-yellow-600/60 ml-2">Sovereign Owner</span>
              </div>
              ...
              {room.moderatorIds && room.moderatorIds.length > 0 && (
                <>
                  <div className="px-4 py-2 bg-green-50/30 border-b border-green-100/50 mt-4">
                     <span className="text-[9px] font-black uppercase tracking-widest text-green-600/60 ml-2">Authority Protocol</span>
                  </div>
              ...
              <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 mt-4">
                 <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 ml-2">Fans Dimension ({filteredFollowers.length})</span>
              </div>
              {isLoading ? (
