...
export function RoomPlayDialog({ 
 ...
 return (
  <Dialog open={open} onOpenChange={handleClose}>
   <DialogContent className="sm:max-w-md bg-[#0a0a0a]/80 backdrop-blur-md border-t border-white/10 p-0 rounded-t-[3rem] overflow-hidden text-white font-sans shadow-2xl animate-in slide-in-from-bottom-full duration-500">
    ...
    {view === 'grid' && (
     <div className="animate-in fade-in duration-500">
      <header className="p-8 pb-4">
       <h2 className="text-2xl font-bold uppercase tracking-tight text-white/90">Room Play</h2>
      </header>
        ...
        <div className="grid grid-cols-3 gap-y-10 gap-x-4">
         {options.map((opt) => (
          <button key={opt.id} onClick={opt.onClick} className="flex flex-col items-center gap-3 active:scale-90 transition-transform">
            ...
            <span className="text-[10px] font-bold uppercase text-white/80 tracking-tight text-center w-full leading-tight">{opt.label}</span>
          </button>
         ))}
        </div>
      </div>
     ...
     <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col h-[65vh]">
      <header className="p-6 pb-2 flex items-center justify-between shrink-0 border-b border-white/5">
        <div className="flex items-center gap-3">
         ...
         <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Music Frequency</h2>
        </div>
      </header>
           ...
           {musicResults.length > 0 ? musicResults.map((video) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
               ...
               <p className="text-[10px] text-white/40 uppercase font-bold tracking-wider mt-1">Global Frequency</p>
              </div>
            </button>
           )) : (
       ...
       <div className="flex-1 flex flex-col overflow-hidden">
         <div className="p-6 pt-2 shrink-0">
          <button 
           ...
           className="w-full h-14 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 border-2 border-white/10 shadow-xl flex items-center justify-center gap-3 font-bold uppercase text-sm active:scale-95 transition-all"
          >
          ...
          <div className="space-y-3 pb-10">
            {roomPlaylist.length > 0 ? roomPlaylist.map((file, idx) => (
             ...
             >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold uppercase text-white truncate">{file.name}</p>
                <div className="flex items-center gap-2 mt-1">
                 <span className="text-[8px] font-bold text-white/40 uppercase tracking-wider">Persistent Sync</span>
                 <div className="h-1 w-1 rounded-full bg-white/20" />
                 <span className="text-[8px] font-bold text-white/40 uppercase tracking-wider">{(file.size / (1024 * 1024)).toFixed(1)} MB</span>
                </div>
              </div>
