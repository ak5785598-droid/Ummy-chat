    ...
    <form onSubmit={handleTransfer}>
      ...
      <div className={cn("space-y-4", !isAuthorized && "opacity-40 grayscale pointer-events-none")}>
       <div className="grid gap-2">
        <div className="flex items-center justify-between px-1">
         <Label htmlFor="recipientId" className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Recipient Tribal ID</Label>
        ...
        <Input
         ...
         className="h-14 rounded-2xl border-2 focus:border-purple-500 transition-all text-xl font-bold text-center text-slate-900 bg-slate-50 placeholder:text-slate-300"
         required
        ...
        {foundRecipient ? (
         <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border-2 border-green-100 w-full animate-in zoom-in duration-300">
          ...
          <div className="flex-1 min-w-0">
           <p className="font-bold text-sm uppercase text-slate-900 truncate">{foundRecipient.username}</p>
         ...
         <div className="flex flex-col items-center justify-center gap-2 opacity-20 ">
          ...
          <p className="text-[10px] font-bold uppercase tracking-wider">Awaiting Valid ID</p>
         </div>
        )}
       </div>

       <div className="grid gap-2">
        <Label htmlFor="amount" className="text-[10px] font-bold uppercase tracking-wider text-gray-400 ml-1">Dispatch Volume</Label>
        <div className="relative">
         ...
         <Input
          ...
          className="h-14 pl-12 rounded-2xl border-2 focus:border-purple-500 transition-all text-2xl font-bold text-slate-900 bg-slate-50 placeholder:text-slate-300"
          required
          disabled={!isAuthorized}
         />
        </div>
        <p className="text-[10px] font-bold text-muted-foreground ml-1 uppercase flex items-center justify-between">
         ...
         {amount && <span className="text-purple-600 font-bold">Syncing: {parseInt(amount).toLocaleString()}</span>}
        </p>
       </div>
      </div>
     ...
     <DialogFooter className="p-8 pt-0">
      <Button 
       ...
       className="w-full h-16 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold uppercase text-xl shadow-xl shadow-purple-500/20 active:scale-95 transition-all"
      >
