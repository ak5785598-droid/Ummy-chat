...
export function RoomGamesDialog({ open, onOpenChange }: RoomGamesDialogProps) {
 ...
 return (
  <Dialog open={open} onOpenChange={onOpenChange}>
   <DialogContent className="w-screen h-screen max-none m-0 rounded-none bg-black/95 backdrop-blur-3xl border-none p-0 flex flex-col text-white font-sans shadow-2xl animate-in slide-in-from-bottom duration-500">
    ...
    <DialogHeader className="p-6 pt-12 border-b border-white/5 flex flex-row items-center justify-between shrink-0 bg-black/40">
     <div className="flex items-center gap-4">
       ...
       <div>
        <DialogTitle className="text-3xl font-bold uppercase tracking-tight">Game Dimension</DialogTitle>
        <DialogDescription className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/40">Synchronize Your Reality</DialogDescription>
       </div>
     </div>
     <button 
    ...
    <ScrollArea className="flex-1">
      <div className="max-w-5xl mx-auto px-8 py-16">
       <div className="flex items-center gap-3 mb-12 px-2">
         ...
         <h3 className="text-xl font-bold uppercase tracking-wider text-white/60">Active Frequencies</h3>
       </div>
          ...
          return (
           ...
           >
            <div className="relative w-full aspect-square">
              <div className="absolute inset-0 bg-primary/10 rounded-3xl blur-2xl opacity-0 group-hover:opacity-100 transition-opacity" />
              
              <button 
               onClick={() => handleGameClick(game.slug)}
               className="relative w-full h-full rounded-3xl overflow-hidden border-2 border-white/10 shadow-2xl group-hover:border-primary transition-all group-hover:shadow-[0_0_40px_rgba(255,204,0,0.2)] bg-white/5 active:scale-95 transform-gpu"
              >
               {displayUrl ? (
                <Image 
                 key={displayUrl}
            ...
            <div className="text-center space-y-2">
              <span className="text-sm font-bold text-white uppercase tracking-[0.2em] group-hover:text-primary transition-colors block">
               {game.title}
              </span>
    ...
    <footer className="p-10 text-center border-t border-white/5 bg-black/60 shrink-0">
      <div className="flex flex-col items-center gap-2">
       <p className="text-[9px] font-bold uppercase tracking-[0.5em] text-white/20">Ummy 3D Graphics Engine Synchronized</p>
