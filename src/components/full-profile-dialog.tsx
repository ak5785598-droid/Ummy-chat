...
export function FullProfileDialog({ 
  ...
  const receivedGifts = profile.stats?.receivedGifts || {}; // Expected: { giftId: count }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent hideClose className="fixed inset-0 translate-x-0 translate-y-0 left-0 top-0 w-full h-full max-w-none bg-white p-0 border-none m-0 rounded-none z-[150] flex flex-col font-outfit overflow-hidden">
        
        {/* Top Section with Background Carousel - Shortened (30vh) */}
          ...
          {images.filter(Boolean).length > 0 ? (
            ...
            <div className="h-full w-full bg-gradient-to-br from-pink-500 via-rose-500 to-pink-600 flex items-center justify-center">
               <div className="flex flex-col items-center gap-2 opacity-50">
                 ...
                 <span className="text-[10px] font-black text-white/40 uppercase tracking-widest text-center">No Space Photos<br/>Add in Edit Profile</span>
               </div>
            </div>
          )}
        ...
        <div className="flex-1 mt-[-40px] relative z-20 bg-white rounded-t-[40px] px-6 pt-0 overflow-y-auto no-scrollbar pb-32">
          ...
          {/* Identity Part (Overlapping - Fixed Cut) */}
          ...
          {/* Stats Bar - Maximum Stretch to Screen Edges */}
          ...
          <div className="mt-2 mb-4">
            ...
            {/* Metadata Badges - Moved here for prominence */}
