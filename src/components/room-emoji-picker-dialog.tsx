...
export function RoomEmojiPickerDialog({ open, onOpenChange, roomId }: RoomEmojiPickerDialogProps) {
  ...
  const handleSendEmoji = (emoji: string) => {
    ...
    // Auto-clear visual frequency after 4 seconds
    setTimeout(() => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[350px] bg-[#121212]/95 backdrop-blur-2xl border-none p-0 rounded-t-[3rem] overflow-hidden text-white font-headline shadow-2xl animate-in slide-in-from-bottom-full duration-500">
        <DialogHeader className="p-6 pb-2">
          <DialogTitle className="text-xl font-black uppercase italic tracking-tighter text-white/90">Reaction Sync</DialogTitle>
          <DialogDescription className="text-[10px] font-bold uppercase tracking-widest text-white/40">Select a vibe to cover your seat</DialogDescription>
        </DialogHeader>
