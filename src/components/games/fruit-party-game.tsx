...
const HandPointer = ({ targetIdx }: { targetIdx: number }) => {
  ...
  // Reduced radius from 155 to 135 to match the closer cards
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-black/60 flex flex-col justify-end z-[100]">
      ...
      >
        ...
        <div className="relative w-full flex-1 flex items-center justify-center scale-95 -translate-y-6">
          ...
          {/* Spreader Lines (Adjusted length to 110) */}
          ...
          {ITEMS.map((item, idx) => {
            ...
            // Cards moved closer to center: from 155 to 135
            const x = Math.cos((angle * Math.PI) / 180) * 135;
            const y = Math.sin((angle * Math.PI) / 180) * 135;
            ...
            return (
              <div key={item.id} className="absolute z-10" style={{ transform: `translate(${x}px, ${y}px)` }}>
                ...
                >
                  ...
                  {/* MIDDLE SECTION: Orange Background (Only if bet exists) */}
                  ...
                  {/* BOTTOM HALF: Orange Background & White Multiplier Text */}
