...
import { doc, increment, collection, onSnapshot, query, orderBy, limit, getDoc, updateDoc, setDoc, Timestamp, getDocs, where } from 'firebase/firestore';
...
// --- MULTIPLIERS (5x, 10x, 15x, 25x, 45x as requested) ---
const ITEMS = [
          ...
          >
              ...
              <div className="tw-top">
                <div className="tw-emoji-box flex items-center justify-center" onClick={() => navigator.vibrate?.(10)}>
                  {winnerData.bet === 0 ? (
                    ...
                    <span className="text-[70px] filter drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)]">{winnerData.emoji}</span>
                  )}
                </div>
              ...
              <div className="tw-winners">
                {[
                  ...
                  <div
                    ...
                    onClick={() => { setActiveWinnerIdx(p.idx); if(p.rank === 1) navigator.vibrate?.(30); }}
            ...
            <style jsx global>{`
              .winning-card {
                ...
                padding: clamp(12px,2vh,18px) clamp(14px,3vw,22px); display: flex; flex-direction: column; justify-content: space-between;
                position: relative; overflow: hidden; box-shadow: 0 0 0 1px rgba(255,255,255,.05) inset, 0 -15px 40px rgba(0,0,0,0.8), 0 -5px 15px rgba(0,0,0,0.6); isolation: isolate;
              }
              .winning-card::before {
              ...
              .tw-stat-row { display: flex; align-items: center; gap: .6rem; background: linear-gradient(180deg, rgba(255,255,255,.07), rgba(255,255,255,.02)); border: 1px solid rgba(255,255,255,.09); padding: .4rem .6rem; border-radius: 12px; font-size: clamp(12px, 2vh, 14px); font-weight: 650; box-shadow: 0 1px 0 rgba(255,255,255,.06) inset, 0 8px 18px rgba(0,0,0,.4); transition: transform .2s, border-color .2s; will-change: transform; }
              ...
              .tw-divider-line { position: absolute; width: 29%; height: 2px; top: 50%; background: linear-gradient(90deg, transparent, #e8c878, transparent); filter: drop-shadow(0 0 8px rgba(232,200,120,.45)); overflow: visible; }
              .tw-divider-line.tw-left { left: 0; transform: scaleX(-1); }
              .tw-divider-line.tw-right { right: 0; }
              .tw-divider-line::after { content: ""; position: absolute; width: 0; height: 0; border-left: 7px solid #e8c878; border-top: 3.5px solid transparent; border-bottom: 3.5px solid transparent; top: -2.5px; right: -6px; filter: drop-shadow(0 0 4px rgba(232,200,120,.7)); animation: arrowPulse 2s ease-in-out infinite; }
              @keyframes arrowPulse { 0%,100%{opacity:.9; transform:translateX(0)} 50%{opacity:1; transform:translateX(2px)} }
              .tw-divider-text { color: #e8c878; font-size: clamp(14px,2.4vh,18px); font-weight: 800; letter-spacing: 1px; text-transform: uppercase; text-shadow: 0 1px 2px #000, 0 0 20px rgba(232,200,120,.4), 0 0 40px rgba(232,200,120,.15); padding: .15rem .7rem; background: radial-gradient(50% 120% at 50% 50%, rgba(232,200,120,.15), transparent 70%); border-radius: 8px; }
              ...
              .tw-player { flex: 1; display: flex; flex-direction: column; align-items: center; text-align: center; cursor: pointer; transition: transform .35s cubic-bezier(.2,.8,.2,1), opacity 0.3s; -webkit-tap-highlight-color: transparent; }
              ...
              .tw-player-name { font-size: clamp(11px,1.8vh,14px); font-weight: 650; margin-top: .7vh; color: #f5f3ef; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 100%; max-width: 118px; transition: color .2s; }
...
export default function CarnivalFoodParty({ onClose, isOverlay = false }: { onClose?: () => void, isOverlay?: boolean }) {
  ...
  // Global game round state
  const [globalRound, setGlobalRound] = useState<any>(null);
  const [globalWinningItemId, setGlobalWinningItemId] = useState<string | null>(null);
  ...
  // Listen to global game round (singleton)
  useEffect(() => {
    if (!firestore) return;
    const roundDocRef = doc(firestore, 'gameRounds', 'currentRound');
    const unsubscribeRound = onSnapshot(roundDocRef, async (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (newStatus === 'betting') {
          ...
          if (data.bettingEndTime) {
            const endTime = data.bettingEndTime.toDate();
            const now = new Date();
          ...
          if (data.spinEndTime) {
            const endTime = data.spinEndTime.toDate();
            const now = new Date();
            const remaining = Math.max(0, Math.floor((endTime.getTime() - now.getTime()) / 1000));
            setSpinTimeLeft(remaining);
          }
          if (!isSpinningRef.current && !moveIntervalRef.current) {
            startLocalSpinAnimation();
          }
        } 
        else if (newStatus === 'result') {
          setGameState('result');
          if (data.winningItemId) {
            setGlobalWinningItemId(data.winningItemId);
            const winningItem = ITEMS.find(item => item.id === data.winningItemId);
            if (winningItem) {
              ...
              if (totalWinAmount > 0 && currentUser && firestore) {
                ...
                addDocumentNonBlocking(collection(firestore, 'globalGameWins'), {
                  gameId: 'fruit-party', amount: totalWinAmount, userId: currentUser.uid, timestamp: new Date(), isGroupWin: false
                });
                const userRef = doc(firestore, 'users', currentUser.uid);
                updateDocumentNonBlocking(userRef, { 'stats.totalWins': increment(totalWinAmount) });
              }
              
              setWinnerData({ emoji: winningItem.icon, win: totalWinAmount, bet: totalMyBetOnWinners, isGroup: false });
              setHistory(prev => [winningItem.icon, ...prev].slice(0, 10));
              Object.entries(myBets).forEach(([itemId, amount]) => {
                const item = ITEMS.find(i => i.id === itemId);
                if (item && amount > 0) {
                  setHistoryData(prev => [{
                    icon: item.icon, bet: amount, time: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
                  }, ...prev]);
                }
              });
  ...
  // Listen to user's bet document for current round
  useEffect(() => {
    if (!firestore || !currentUser || !globalRound) return;
    const userBetRef = doc(firestore, 'gameRounds', 'currentRound', 'bets', currentUser.uid);
    const unsubscribeBet = onSnapshot(userBetRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setMyBets(data.bets || {});
      } else {
    ...
    return () => unsubscribeBet();
  }, [firestore, currentUser, globalRound]);

  // Countdown timer for betting phase
  useEffect(() => {
    if (!globalRound || gameState !== 'betting') return;
  ...
  }, [globalRound, gameState]);

  // Countdown timer for spinning phase
  useEffect(() => {
    if (!globalRound || gameState !== 'spinning') return;
    const interval = setInterval(() => {
  ...
  // Fetch & compute global top winners
  useEffect(() => {
    if (!firestore) return;
    const unsubscribe = onSnapshot(collection(firestore, 'globalGameWins'), async (snapshot) => {
      ...
      for (const entry of sorted) {
        try {
          ...
          winnersData.push({ name: displayName, win: entry.totalWin, avatar: photoURL, bet: 0, isMe: currentUser?.uid === entry.userId });
        } catch (err) {
          winnersData.push({ name: 'Unknown', win: entry.totalWin, avatar: null, bet: 0, isMe: false });
        }
      }
      const finalWinners: WinnerPlayer[] = [...winnersData];
      while (finalWinners.length < 3) {
        finalWinners.push({ name: 'Waiting...', win: 0, avatar: null, bet: 0, isMe: false });
      }
  ...
  const createInitialRound = async () => {
    if (!firestore) return;
    const roundDocRef = doc(firestore, 'gameRounds', 'currentRound');
    const now = Timestamp.now();
    const bettingEndTime = Timestamp.fromDate(new Date(now.toDate().getTime() + 30 * 1000));
    await setDoc(roundDocRef, { roundId: Date.now(), status: 'betting', bettingEndTime, createdAt: now });
  };
  ...
  const createNextRound = async () => {
    ...
    const bettingEndTime = Timestamp.fromDate(new Date(now.toDate().getTime() + 30 * 1000));
    await setDoc(roundDocRef, { roundId: Date.now(), status: 'betting', bettingEndTime, createdAt: now });
    setGlobalWinningItemId(null);
    setWinnerData(null);
    setMyBets({});
  };

  const startLocalSpinAnimation = () => {
    if (moveIntervalRef.current) clearInterval(moveIntervalRef.current);
    if (countdownIntervalRef.current) clearInterval(countdownIntervalRef.current);
    ...
    const checkSpinEnd = setInterval(() => {
      if (globalRound?.spinEndTime) {
        const endTime = globalRound.spinEndTime.toDate();
        if (new Date() >= endTime) {
          clearInterval(checkSpinEnd);
          if (moveIntervalRef.current) clearInterval(moveIntervalRef.current);
          if (countdownIntervalRef.current) clearInterval(countdownIntervalRef.current);
          isSpinningRef.current = false;
          if (globalRound.winningItemId) {
            const winIdx = ITEMS.findIndex(i => i.id === globalRound.winningItemId);
            setHighlightIdxs([winIdx]);
          }
        }
      }
    }, 100);
  ...
  const handlePlaceBet = async (id: string) => {
    if (gameState !== 'betting' || !currentUser || !firestore || isSubmittingBet) {
    ...
    try {
      ...
      const newBets = { ...myBets, [id]: (myBets[id] || 0) + selectedChip };
      setMyBets(newBets);
      const userProfileRef = doc(firestore, 'users', currentUser.uid, 'profile', currentUser.uid);
      await updateDocumentNonBlocking(userProfileRef, { 'wallet.coins': increment(-selectedChip) });
      const userBetRef = doc(firestore, 'gameRounds', 'currentRound', 'bets', currentUser.uid);
      await setDoc(userBetRef, { bets: newBets }, { merge: true });
    } catch (error) {
      console.error("Error placing bet:", error);
      alert("Failed to place bet. Please try again.");
      setLocalCoins(prev => prev + selectedChip);
      setMyBets(prev => ({ ...prev, [id]: Math.max(0, (prev[id] || 0) - selectedChip) }));
    } finally {
  ...
  const winnersListForPopup = globalTopWinners;
  const winnerPopupData = winnerData ? { emoji: winnerData.emoji, win: winnerData.win, bet: winnerData.bet } : null;
      ...
      <AnimatePresence mode="wait">
        <motion.div 
            ...
            className={cn(
              ...
              !isOverlay && "min-h-[80vh]", "rounded-none"
            )}
            style={{ backgroundImage: 'radial-gradient(circle at top, #1e3a8a, #020617)', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}
          >
            {/* HEADER - No yellow backgrounds on icons */}
            <div className="w-full flex flex-col z-20">
              <div className="w-full p-4 flex justify-between items-center relative">
                <div className="flex items-center gap-2">
                  <button onPointerDown={(e) => dragControls.start(e)} className="w-8 h-8 rounded-full bg-gray-800/90 border border-gray-600 flex items-center justify-center text-white cursor-grab active:cursor-grabbing touch-none shadow-md hover:scale-105 transition-transform">
                  ...
                  <div className="relative flex items-center bg-gradient-to-br from-[#1e2350]/90 to-[#0d1030] border border-amber-400/60 rounded-full h-7 min-w-[105px] ml-2 shadow-[inset_0_1px_2px_rgba(0,0,0,0.4),0_2px_4px_rgba(0,0,0,0.2)]">
                    <div className="absolute -left-1 w-8 h-8 flex items-center justify-center z-10 drop-shadow-md"><DollarCoin className="w-8 h-8" /></div>
                ...
                <div className="flex items-center gap-2 relative">
                  <div className="absolute top-12 right-2 z-10 pointer-events-none drop-shadow-[0_4px_4px_rgba(0,0,0,0.3)]"><Cloud className="w-28 h-auto" /></div>
                  {[ { icon: Clock, action: () => setShowHistoryPage(true) }, { icon: isSoundOn ? Volume2 : VolumeX, action: () => setIsSoundOn(!isSoundOn) }, { icon: HelpCircle, action: () => setShowRules(true) }, { icon: X, action: onClose } ].map((btn, i) => (
                    <button key={i} onClick={btn.action} className="w-8 h-8 rounded-full bg-gray-800/90 border border-gray-600 flex items-center justify-center text-white shadow-md hover:scale-105 transition-transform">
              ...
              <div className="px-4 mt-1 relative">
                ...
                <div className="absolute top-10 left-6 z-10 pointer-events-none drop-shadow-[0_4px_4px_rgba(0,0,0,0.3)]"><Cloud className="w-14 h-auto" /></div>
              </div>
            </div>
            ...
            <div className="relative w-full flex-1 flex items-center justify-center scale-95 -translate-y-6" style={{ perspective: '1000px' }}>
              <svg className="absolute w-full h-full pointer-events-none z-0 overflow-visible">
                <defs><linearGradient id="darkWoodGradient" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#3d1a05" /><stop offset="100%" stopColor="#1a0801" /></linearGradient></defs>
              ...
              {ITEMS.map((item, idx) => {
                ...
                return (
                  <div key={item.id} className="absolute z-10" style={{ transform: `translate(${x}px, ${y}px)` }}>
                    <button onClick={() => handlePlaceBet(item.id)} style={{ transform: `rotateY(${isHighlighted ? '0deg' : '15deg'}) rotateX(10deg)` }} className={cn("w-24 h-24 rounded-full border-[4px] border-yellow-500 flex flex-col overflow-hidden transition-all duration-300 relative items-center justify-center", "bg-[#7f1d1d] shadow-[10px_10px_20px_rgba(0,0,0,0.5)]", isHighlighted ? "scale-110 -translate-y-2 ring-[6px] ring-[#ffd700] shadow-[0_0_40px_#ffd700] z-50" : "")}>
                      <div className="w-full flex-[1.2] flex items-center justify-center"><span className="text-4xl drop-shadow-lg z-20">{item.icon}</span></div>
                      <AnimatePresence>{betAmount > 0 && (<motion.div initial={{ height: 0 }} animate={{ height: "auto" }} className="w-full bg-orange-500 flex items-center justify-center py-0.5 z-20 gap-1"><DollarCoin className="w-3 h-3" /><span className="text-[10px] font-black text-white">{betAmount >= 1000 ? `${betAmount/1000}K` : betAmount}</span></motion.div>)}</AnimatePresence>
                      <div className="w-full flex-1 flex items-center justify-center bg-orange-600 z-20"><span className="font-black text-[12px] text-white italic">×{item.multiplier}</span></div>
                    </button>
                  </div>
                );
              })}
            ...
            <div className="w-full px-4 mb-2 z-20 relative">
              ...
              <div className="w-full h-12 bg-[#3e1a05] rounded-xl border-2 border-[#f5d0a9] flex items-center px-4 gap-3 overflow-x-auto no-scrollbar">
                ...
                {history.map((icon, i) => (<div key={i} className="min-w-[32px] h-8 bg-black/30 rounded-lg flex items-center justify-center text-lg">{icon}</div>))}
              </div>
            </div>
            ...
            {showRules && (
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="absolute bottom-0 left-0 right-0 h-[40vh] bg-[#0ea5e9] border-t-[10px] border-[#0284c7] z-[300] flex flex-col px-6 py-8">
                ...
                <div className="text-white/95 font-semibold text-sm space-y-4">
                  ...
                  <p className="text-yellow-200 font-bold">4. Global round: All players bet together, same winner for everyone!</p>
                </div>
              </motion.div>
            )}
                  ...
                  <div className="bg-white h-full rounded-[2.5rem] p-4 overflow-y-auto no-scrollbar">
                    {historyData.map((rec, i) => (
                      <div key={i} className="flex items-center justify-between bg-gray-50 p-3 mb-2 rounded-2xl">
                        ...
                        <div className="flex items-center gap-1 font-black text-gray-800"><DollarCoin className="w-4 h-4" /><span>{rec.bet.toLocaleString()}</span></div>
