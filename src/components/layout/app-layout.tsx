...
export function AppLayout({ 
 ...
 ), [pathname, isOfficial, handleLogout, t]);
 ...
 }, [pathname, shouldShowBottomNav, t]);
 ...
 if (userProfile?.banStatus?.isBanned) {
  ...
  try {
   if (userProfile?.banStatus?.bannedUntil) {
    until = typeof userProfile.banStatus.bannedUntil.toDate === 'function' ? userProfile.banStatus.bannedUntil.toDate() : new Date(userProfile.banStatus.bannedUntil);
   }
  } catch (e) {}
  if (!until || isNaN(until.getTime()) || until > new Date()) {
   return <BanDialog isOpen={true} onClose={handleLogout} bannedUntil={userProfile.banStatus.bannedUntil} accountNumber={userProfile.accountNumber} />;
  }
 }

 // Screen Type Guard
 if (fullScreen || pathname?.startsWith('/login') || pathname === '/' || pathname === '/terms' || pathname === '/privacy-policy' || pathname === '/refund-policy' || pathname === '/contact' || pathname === '/help-center') {
