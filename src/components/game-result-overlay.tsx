...
export function GameResultOverlay({ gameId, winningSymbol, winAmount, winners: propWinners }: GameResultOverlayProps) {
 ...
 return (
  <div className="fixed inset-0 z-[300] flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in zoom-in duration-500 font-sans p-4 select-none pointer-events-auto">
   <div className="w-full max-w-sm flex flex-col items-center">
    
    <div className="mb-4 relative">
      <h2 className="text-7xl font-bold text-yellow-400 uppercase tracking-tight drop-shadow-[0_6px_0_#8b4513] text-center animate-bounce">
       YOU WIN
      </h2>
    </div>
      ...
      <div className="bg-[#5d1a2a] rounded-2xl border-4 border-[#fbbf24]/40 shadow-2xl flex items-center h-20 px-6 overflow-hidden relative">
       <div className="flex items-center gap-4 flex-1">
         ...
         <div className="flex items-center gap-2">
          ...
          <span className="text-3xl font-bold text-white tracking-tight tabular-nums drop-shadow-lg">
    ...
    <div className="w-full bg-[#4a1424] rounded-3xl border-2 border-[#fbbf24]/20 shadow-2xl overflow-hidden p-2">
      <div className="divide-y divide-white/5">
          ...
          <div className="flex items-center gap-4">
           ...
           <Avatar className={cn(
            ...
            idx === 0 ? "border-yellow-400" : idx === 1 ? "border-blue-300" : "border-orange-400"
             ...
             <AvatarFallback className="bg-slate-800 text-white font-bold text-xs">{(winner.name || 'U').charAt(0)}</AvatarFallback>
           </Avatar>
           <span className="text-sm font-bold text-white uppercase tracking-tight truncate w-28">
             {winner.name}
           </span>
          </div>
       ...
       {displayWinners.length === 0 && (
        <div className="py-12 text-center opacity-40">
          <p className="text-xs font-bold uppercase tracking-wider text-white/60">Awaiting Real Winners...</p>
        </div>
       )}
      </div>
    </div>
