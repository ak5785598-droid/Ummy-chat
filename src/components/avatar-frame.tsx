      ...
      return (
        <div className="absolute inset-0 w-full h-full rounded-full p-[2px] overflow-visible">
          ...
          <div className="absolute inset-0 rounded-full border-[4px] border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.8)]" 
               style={{ background: 'linear-gradient(135deg, #FFF281 0%, #FFB700 50%, #B8860B 100%)' }} />
          {/* Glowing Crown Icon */}
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-30 drop-shadow-[0_0_10px_#FFD700]">
      ...
      return (
        <div className="absolute inset-0 w-full h-full rounded-full p-[2px] overflow-visible">
          ...
          <div className="absolute inset-0 rounded-full border-[3px] border-cyan-200 shadow-[0_0_20px_rgba(34,211,238,0.6)]"
               style={{ background: 'linear-gradient(45deg, #E0F2FE 0%, #7DD3FC 50%, #0EA5E9 100%)' }} />
          {/* Floating Gems */}
          {[0, 90, 180, 270].map((deg) => (
            <motion.div
              ...
              className="absolute w-2 h-2 bg-white rotate-45 border border-cyan-400 z-30 shadow-[0_0_5px_#fff]"
              style={{ 
      ...
      return (
        <div className="absolute inset-0 w-full h-full rounded-full overflow-visible">
          ...
          <motion.div
             ...
             style={{ background: 'radial-gradient(circle, #A855F7 0%, #6366F1 50%, #1E1B4B 100%)' }}
          />
          {/* Neon Pulses */}
          <motion.div
...
export function AvatarFrame({ frameId, children, className, size = 'md' }: AvatarFrameProps) {
  ...
  return (
    <div className={cn('relative flex items-center justify-center', className)}>
      {/* 
          ELITE FRAME LAYER 
          Z-index 20 ensures it stays above the avatar image.
      */}
      <div className="absolute inset-[-15%] z-20 pointer-events-none flex items-center justify-center overflow-visible">
         <EliteFrameRenderer frameId={frameId} />
      </div>

      {/* CORE AVATAR IMAGE */}
      <div className="relative z-10 rounded-full overflow-hidden w-full h-full bg-slate-900 ring-2 ring-white/10">
        {children}
      </div>
    </div>
  );
