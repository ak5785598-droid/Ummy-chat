  ...
  return (
    <div className="mt-8 mb-4 flex flex-col items-center gap-1.5 animate-in fade-in slide-in-from-top-2 duration-1000">
       <div className="bg-black/20 backdrop-blur-md px-4 py-1 rounded-full border border-white/5 flex items-center gap-2">
          ...
          <span className="text-[8px] font-black uppercase text-white/40 tracking-[0.3em] italic">Reset Sync (GMT+5:30)</span>
       </div>
       <span className="text-2xl font-black text-white italic drop-shadow-lg tabular-nums tracking-tighter uppercase">{timeLeft}</span>
    </div>
  );
};
    ...
    <div className="space-y-4 animate-in fade-in duration-1000 relative pb-40">
      <div className="relative pt-4 flex flex-col items-center">
        ...
        <div className="flex items-end justify-center gap-3 w-full max-sm px-2 relative z-20">
           {top2 && (
             <Link href={type === 'rooms' ? `/rooms/${top2.id}` : `/profile/${top2.id}`} className="flex-1 bg-gradient-to-b from-[#252b41] to-[#1a1f30] rounded-[2rem] border-2 border-blue-400/20 p-3 pt-10 flex flex-col items-center gap-1.5 shadow-2xl relative transition-all active:scale-95 group">
           ...
           {top3 && (
             <Link href={type === 'rooms' ? `/rooms/${top3.id}` : `/profile/${top3.id}`} className="flex-1 bg-gradient-to-b from-[#2d221a] to-[#1f1610] rounded-[2rem] border-2 border-amber-400/20 p-3 pt-10 flex flex-col items-center gap-1.5 shadow-2xl relative transition-all active:scale-95 group">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20">
        ...
        {others.map((item, index) => (
          <Link key={item.id} href={type === 'rooms' ? `/rooms/${item.id}` : `/profile/${item.id}`} className="flex items-center gap-2.5 p-2 bg-white/5 backdrop-blur-sm rounded-xl border border-white/5 group hover:bg-white/10 transition-all active:scale-[0.98]">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-black text-[11px] uppercase text-yellow-500 truncate tracking-tight mb-0.5">{getDisplayName(item)}</p>
            ...
            <div className="text-right flex items-center gap-1 shrink-0">
              <span className="font-black text-[11px] text-white/80 italic">{formatValue(getValue(item))}</span>
    ...
    <div className="min-h-screen bg-[#050505] text-white relative font-headline overflow-x-hidden flex flex-col">
        ...
        <footer className="fixed bottom-0 left-0 right-0 z-[100] bg-gradient-to-r from-[#b88a44] via-[#f5e1a4] to-[#b88a44] p-2.5 h-14 flex items-center shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
           <div className="max-w-4xl mx-auto flex items-center gap-3 w-full">
              ...
              <div className="text-right flex items-center gap-1 shrink-0">
                <span className="text-lg font-black text-black italic leading-none">{myValue.toLocaleString()}</span>
