 ...
 return (
  ...
  >
   <div className="flex items-center gap-4">
    ...
    <span className="font-bold text-gray-800 text-sm uppercase tracking-tight">{label}</span>
   </div>
   <div className="flex items-center gap-2">
    {extra && <span className="text-[10px] font-bold text-muted-foreground uppercase">{extra}</span>}
    ...
    {view === 'main' ? (
     <>
      <header className="px-6 pt-10 pb-10 pt-safe">
        ...
        <h1 className="mt-4 text-3xl font-bold uppercase tracking-tight text-slate-900">{t.settings.title}</h1>
      </header>

      <div className="px-6 space-y-10">
        <section className="space-y-4">
         <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-2">{t.settings.identity}</p>
         <Card className="rounded-3xl border-2 border-gray-50 shadow-sm overflow-hidden bg-white divide-y divide-gray-50">
           <MenuItem 
            icon={LinkIcon} 
        ...
        <section className="space-y-6 flex flex-col items-center">
         <button 
          ...
          className="w-full h-20 rounded-2xl bg-red-50 text-red-600 border-2 border-red-100 hover:bg-red-100 shadow-none font-bold uppercase text-xl flex items-center justify-center gap-4 transition-all active:scale-95"
         >
         ...
         <button 
          ...
          className="flex items-center gap-2 text-red-400 font-bold uppercase text-xs hover:text-red-500 transition-colors py-4"
         >
        ...
        <p className="text-center text-[8px] font-bold uppercase tracking-[0.3em] text-gray-300 mt-4 ">
         {t.settings.footer}
        </p>
      </div>
     </>
     ...
     <div className="animate-in slide-in-from-right duration-300 flex flex-col h-[100dvh]">
       <header className="px-6 pt-10 pb-6 flex items-center justify-between sticky top-0 bg-white z-50 pt-safe">
        ...
        <h1 className="text-xl font-bold uppercase tracking-tight">Connections</h1>
        <div className="w-10" />
       </header>
       ...
       <div className="p-6 space-y-6 flex-1 overflow-y-auto pb-32">
        ...
        <Card className="rounded-3xl border-2 border-gray-50 shadow-sm overflow-hidden bg-white p-6 space-y-6 divide-y divide-gray-50">
          {/* Google */}
          <div className="flex items-center justify-between pt-2 pb-2">
           <div className="flex items-center gap-4">
            <div className="p-3 bg-gray-50 rounded-2xl"><FcGoogle className="h-6 w-6" /></div>
            <div className="flex flex-col">
             <span className="font-bold text-gray-800 uppercase ">Google</span>
             {linkedProviders.includes('google.com') ? (
              <span className="text-[10px] font-bold text-green-500 uppercase tracking-wider flex items-center gap-1"><ShieldCheck className="h-3 w-3" /> Connected</span>
             ) : (
           ...
           {linkedProviders.includes('google.com') ? (
            ...
            <Button size="sm" onClick={handleLinkGoogle} disabled={isLinking} className="rounded-full font-bold uppercase text-[10px]">Connect</Button>
           )}
          </div>
          ...
          <div className="flex items-center justify-between pt-6 pb-2">
           <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
             <span className="font-bold text-gray-800 uppercase ">Facebook</span>
           ...
           {linkedProviders.includes('facebook.com') ? (
            ...
            <Button size="sm" onClick={handleLinkFacebook} disabled={isLinking} className="bg-blue-600 hover:bg-blue-700 rounded-full font-bold uppercase text-[10px]">Connect</Button>
           )}
          </div>
          ...
          <div className="flex items-center justify-between pt-6 pb-2">
           <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
             <span className="font-bold text-gray-800 uppercase ">Phone</span>
           ...
           <div className="pt-6 flex flex-col gap-3">
            {phoneLoginStep === 'number' ? (
             <>
              ...
              <Button onClick={handleLinkPhone} disabled={isLinking || !phoneNumber} className="h-12 w-full rounded-xl bg-green-600 hover:bg-green-700 font-bold uppercase text-xs">Connect Phone</Button>
             </>
            ) : (
             <>
              <p className="text-[10px] text-center text-gray-500 uppercase font-bold">Code sent to {phoneNumber}</p>
              <Input type="text" maxLength={6} placeholder="000000" value={verificationCode} onChange={(e) => setVerificationCode(e.target.value)} disabled={isLinking} className="h-12 rounded-xl text-center font-bold tracking-[0.2em] text-lg bg-white border-2 border-gray-100" />
              <Button onClick={handleVerifyPhoneCode} disabled={isLinking || verificationCode.length < 6} className="h-12 w-full rounded-xl bg-green-600 hover:bg-green-700 font-bold uppercase text-xs">Verify Code</Button>
     ...
     <div className="animate-in slide-in-from-right duration-300 flex flex-col h-full">
      <header className="px-6 pt-10 pb-6 flex items-center justify-between sticky top-0 bg-white z-50 pt-safe">
        ...
        <h1 className="text-xl font-bold uppercase tracking-tight">{t.settings.langSelect}</h1>
        <div className="w-10" />
      </header>
         ...
         {LANGUAGES.map((lang) => (
          ...
          >
            <div className="flex flex-col items-start">
             <span className="font-bold text-gray-800 text-lg uppercase tracking-tight">{lang.name}</span>
             <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{lang.native}</span>
            </div>
            {language === lang.id && (
