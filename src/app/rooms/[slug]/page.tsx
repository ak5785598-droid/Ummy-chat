...
import { Loader, Ghost, Ban, Lock, ArrowRight } from 'lucide-react';
...
export default function RoomPage({ params }: { params: Promise<{ slug: string }> }) {
 ...
 const activeRoom: Room | null = useMemo(() => {
  if (firestoreRoom) {
   return {
    ...
    musicUpdatedBy: firestoreRoom.musicUpdatedBy,
  ...
  return (
   <AppLayout>
    <div className="flex h-[60vh] flex-col items-center justify-center space-y-6 text-center px-6">
     ...
     <h1 className="text-3xl font-bold uppercase ">Frequency Exclusion</h1>
  ...
  return (
   <AppLayout>
    <div className="flex h-[60vh] flex-col items-center justify-center text-center px-6">
      ...
      <h1 className="text-2xl font-bold uppercase">Frequency Not Found</h1>
      <button onClick={() => router.push('/rooms')} className="mt-6 bg-primary text-white px-10 py-3 rounded-full font-bold uppercase ">Back to Home</button>
    </div>
   </AppLayout>
  );
 }
 ...
 if (requiresPassword) {
  return (
   <AppLayout fullScreen>
    <div className="fixed inset-0 bg-[#FFCC00] z-[1000] flex flex-col items-center justify-center p-8 font-sans">
      <div className="mb-8 flex flex-col items-center text-center gap-4 animate-in fade-in zoom-in duration-700">
       ...
       <div className="space-y-1">
         <h2 className="text-3xl font-bold uppercase tracking-tight text-black">{activeRoom.title}</h2>
         <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-black/60">Private Frequency Active</p>
       </div>
      </div>
      ...
      <div className="w-full max-w-xs space-y-8 animate-in slide-in-from-bottom-10 duration-700">
       <div className="space-y-4 text-center">
         <Input 
          ...
          className="h-16 bg-white border-none rounded-[1.25rem] shadow-xl text-3xl font-bold tracking-[1em] text-center focus:ring-4 focus:ring-black/10 placeholder:text-black/5"
          placeholder="0000"
          autoFocus
         />
         <p className="text-[10px] font-bold uppercase tracking-wider text-black/40">Enter 4-Digit Entry Code</p>
       </div>

       <div className="flex gap-4">
         <button 
          onClick={() => router.push('/rooms')}
          className="flex-1 h-12 rounded-xl bg-white/20 border border-black/10 font-bold uppercase text-xs hover:bg-white/30 transition-all"
         >
          Exit
         </button>
         <Button 
          ...
          className="flex-[2] h-12 rounded-xl bg-black text-white font-bold uppercase text-base shadow-2xl hover:scale-[1.02] active:scale-95 transition-all"
         >
