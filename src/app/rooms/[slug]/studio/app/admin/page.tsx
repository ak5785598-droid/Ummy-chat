...
export default function AdminPage() {
 ...
 const SearchToggle = ({ mode, setMode }: { mode: 'id' | 'name', setMode: (m: 'id' | 'name') => void }) => (
  <div className="flex bg-slate-100 p-1 rounded-xl w-fit">
    <button onClick={() => setMode('id')} className={cn("px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all", mode === 'id' ? "bg-white text-slate-900 shadow-sm" : "text-slate-400")}>By ID</button>
    <button onClick={() => setMode('name')} className={cn("px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all", mode === 'name' ? "bg-white text-slate-900 shadow-sm" : "text-slate-400")}>By Name</button>
  </div>
 );

 if (!isCreator) return <AppLayout><div className="flex h-[50vh] items-center justify-center text-destructive font-sans"><Shield className="h-12 w-12 mr-2" /> Portal Access Restricted</div></AppLayout>;
   ...
   <div className="space-y-8 max-w-7xl mx-auto p-4 animate-in fade-in duration-700 font-sans bg-white min-h-full">
    <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6">
     <div className="flex items-center gap-4">
       ...
       <div><h1 className="text-4xl font-bold uppercase tracking-tight text-slate-900">Supreme Command</h1><p className="text-muted-foreground">Supreme Authority Protocol Active.</p></div>
     </div>
     <Badge className="bg-primary text-black font-bold uppercase px-4 py-1.5 h-10 rounded-xl">Supreme Creator</Badge>
    </header>
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col md:flex-row gap-10 items-start">
     <div className="w-full md:w-72 shrink-0 md:sticky md:top-24 h-fit">
      <ScrollArea className="h-[calc(100vh-200px)] pr-4">
       <TabsList className="flex flex-col h-fit w-full bg-slate-50 shadow-2xl rounded-3xl border border-slate-100 p-3 gap-2 overflow-visible">
        <TabsTrigger value="app-data" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Database className="h-4 w-4" /> App Ledger
        </TabsTrigger>
        <TabsTrigger value="themes" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Palette className="h-4 w-4" /> Theme Hub
        </TabsTrigger>
        <TabsTrigger value="tags" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <BadgeCheck className="h-4 w-4" /> Assign Tags
        </TabsTrigger>
        <TabsTrigger value="assign-center" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <ShieldCheck className="h-4 w-4" /> Assign Center
        </TabsTrigger>
        <TabsTrigger value="authority" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Zap className="h-4 w-4" /> Authority Hub
        </TabsTrigger>
        <TabsTrigger value="app-branding" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Palette className="h-4 w-4" /> App Branding
        </TabsTrigger>
        <TabsTrigger value="pin-control" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Pin className="h-4 w-4" /> Pin Control
        </TabsTrigger>
        <TabsTrigger value="member-directory" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Users className="h-4 w-4" /> Member Directory
        </TabsTrigger>
        <TabsTrigger value="ranking-themes" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Trophy className="h-4 w-4" /> Ranking Themes
        </TabsTrigger>
        <TabsTrigger value="user-records" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <UserSearch className="h-4 w-4 text-rose-500" /> User Ledger
        </TabsTrigger>
        <TabsTrigger value="id-ban" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Gavel className="h-4 w-4" /> ID Ban Control
        </TabsTrigger>
        <TabsTrigger value="banners" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <ImageIcon className="h-4 w-4" /> Banners
        </TabsTrigger>
        <TabsTrigger value="games" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Gamepad2 className="h-4 w-4" /> Game Sync
        </TabsTrigger>
        <TabsTrigger value="broadcaster" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <Megaphone className="h-4 w-4" /> Broadcaster
        </TabsTrigger>
        <TabsTrigger value="direct-messenger" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
         <MessageSquareText className="h-4 w-4" /> Direct Messenger
        </TabsTrigger>

        <TabsTrigger value="rewards" className="w-full justify-start h-14 rounded-2xl px-6 font-bold uppercase text-xs gap-3 text-slate-600 data-[state=active]:bg-primary data-[state=active]:text-white">
     ...
     <div className="flex-1 w-full min-w-0">
      <TabsContent value="app-data" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0 flex flex-row items-center justify-between">
           <div>
            <CardTitle className="text-2xl uppercase flex items-center gap-2 text-blue-600"><BarChart3 className="h-6 w-6" /> App Economic Ledger</CardTitle>
         ...
         <CardContent className="px-0 space-y-10">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-6 bg-blue-50 rounded-3xl border-2 border-blue-100 flex flex-col gap-1">
              <p className="text-[10px] font-bold uppercase text-blue-400 tracking-wider">Total Coins in Tribe</p>
            ...
            <div className="p-6 bg-cyan-50 rounded-3xl border-2 border-cyan-100 flex flex-col gap-1">
              <p className="text-[10px] font-bold uppercase text-cyan-400 tracking-wider">Total Diamonds Accumulated</p>
            ...
            <div className="p-6 bg-purple-50 rounded-3xl border-2 border-purple-100 flex flex-col gap-1">
              <p className="text-[10px] font-bold uppercase text-purple-400 tracking-wider">Total Economic Output (Spent)</p>
            ...
            <div className="p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 flex flex-col gap-1">
              <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Total Synchronized Users</p>
        ...
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         ...
         <div className="flex flex-col gap-4">
           <div className="flex gap-4">
            ...
            />
            <Button onClick={handleRoomPinSearch} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingRoomPin}>Find Frequency</Button>
           </div>
         </div>
          ...
          <div className="mt-10 p-8 border-2 rounded-3xl space-y-8 animate-in slide-in-from-bottom-4 bg-slate-50/20">
            <div className="flex items-center justify-between border-b pb-6">
             <div className="flex items-center gap-4">
               ...
               <div>
                <p className="font-bold uppercase text-xl tracking-tight text-slate-900">{targetRoomForPin.name || targetRoomForPin.title}</p>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Room ID: {targetRoomForPin.roomNumber}</span>
               </div>
             </div>
             <div className="text-right">
               <p className="text-[10px] font-bold uppercase text-gray-400 mb-1">Pin Frequency</p>
               {targetRoomForPin.isPinned ? (
                <Badge className="bg-emerald-500 text-white font-bold uppercase text-[10px] py-1 px-3">PINNED ACTIVE</Badge>
               ) : (
                <Badge className="bg-slate-200 text-slate-400 font-bold uppercase text-[10px] py-1 px-3 shadow-none">NOT PINNED</Badge>
               )}
             </div>
            </div>
      ...
      <TabsContent value="app-branding" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0">
           <CardTitle className="text-2xl uppercase flex items-center gap-2 text-pink-600"><Palette className="h-6 w-6" /> App Visual Branding</CardTitle>
           ...
           <div className="p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
               ...
               <span className="font-bold uppercase text-sm text-slate-900">Global Room Notice (Row 1)</span>
              </div>
              <Badge className="bg-orange-100 text-orange-600 border-none font-bold text-[8px] uppercase">All Rooms Sync</Badge>
            </div>
            <div className="flex gap-2">
              <Input 
               ...
               className="h-14 rounded-2xl border-2 bg-white font-bold shadow-sm"
              />
              <Button 
           ...
           <div className="p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
               ...
               <span className="font-bold uppercase text-sm text-slate-900">Login Page Background</span>
              </div>
              {config?.loginBackgroundUrl && (
               <Button variant="ghost" size="sm" className="text-[8px] font-bold uppercase text-red-500" onClick={() => updateDoc(configRef!, { loginBackgroundUrl: null })}>Reset to Default</Button>
              )}
            </div>
            ...
            <div className="relative aspect-video rounded-3xl overflow-hidden bg-slate-900 border-2 border-white shadow-inner flex items-center justify-center">
              {config?.loginBackgroundUrl ? (
               ...
               <div className="flex flex-col items-center justify-center gap-2 text-white/20">
                ...
                <span className="uppercase font-bold text-[10px] tracking-wider">Stars Default Active</span>
               </div>
              )}
      ...
      <TabsContent value="ranking-themes" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0">
           <CardTitle className="text-2xl uppercase flex items-center gap-2 text-yellow-600"><Trophy className="h-6 w-6" /> Ranking Environmental Themes</CardTitle>
         ...
         <CardContent className="px-0 grid grid-cols-1 md:grid-cols-2 gap-8">
           {[
            ...
            <div key={item.key} className="p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 space-y-4">
             <div className="flex items-center justify-between">
               <div className="flex items-center gap-2">
               ...
               {rankingConfig?.[item.key] && (
                <Button variant="ghost" size="sm" className="text-[8px] font-bold uppercase text-red-500" onClick={() => updateDoc(rankingConfigRef!, { [item.key]: null })}>Reset</Button>
               )}
             </div>
             ...
             <div className="relative aspect-video rounded-2xl overflow-hidden bg-slate-200 border-2 border-white shadow-inner flex items-center justify-center">
               {rankingConfig?.[item.key] ? (
                ...
                <div className="text-center opacity-20"><ImageIcon className="h-8 w-8 mx-auto mb-1" /><span className="text-[8px] font-bold uppercase">Nebula Default Active</span></div>
               )}
      ...
      <TabsContent value="authority" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-gradient-to-br from-primary/10 to-transparent">
         <CardHeader><CardTitle className="text-2xl uppercase flex items-center gap-2 text-primary"><Zap className="h-6 w-6" /> Zap Authority Protocol</CardTitle></CardHeader>
             ...
             <div key={u.id} className="p-4 bg-white rounded-2xl border flex flex-col gap-4 shadow-sm">
               <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-bold text-sm uppercase text-slate-900">{u.username}</p>
                  <p className="text-[10px] text-muted-foreground">ID: {u.accountNumber || u.id.slice(0, 6)}</p>
                </div>
                <div className="flex gap-2">
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {AUTHORITY_ROLES.map(role => (
                 <Button key={role.id} variant={u.tags?.includes(role.id) ? 'default' : 'outline'} size="sm" onClick={() => toggleUserRole(u.id, role.id, u.tags)} className="h-10 text-[8px] font-bold uppercase rounded-xl">{role.label}</Button>
                ))}
               </div>
             </div>
            ))}
      ...
      <TabsContent value="member-directory" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0 flex flex-row items-center justify-between">
           <div>
            <CardTitle className="text-2xl uppercase flex items-center gap-2 text-slate-900"><Users className="h-6 w-6" /> Tribal Member Archive</CardTitle>
         ...
         <CardContent className="px-0">
           <div className="bg-slate-50 rounded-3xl border border-slate-100 overflow-hidden divide-y divide-slate-200">
            {tribalMembers.length === 0 ? (
             <div className="py-40 text-center opacity-20 ">Awaiting Synchronized Directory...</div>
             ...
             <div key={member.id} className="p-6 flex items-center justify-between hover:bg-slate-100/50 transition-colors">
               <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-sm uppercase text-slate-900">{member.username}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                   <span className="text-[10px] font-bold text-slate-400">ID: {member.accountNumber}</span>
                   <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Spent: {member.wallet?.totalSpent.toLocaleString() || 0}</span>
                  </div>
                </div>
               </div>
               <div className="text-right">
                ...
                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider mt-1">Status: {member.isOnline ? 'ONLINE' : 'STARDUST'}</p>
               </div>
             </div>
            ))}
           </div>
      ...
      <TabsContent value="user-records" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0">
           <CardTitle className="text-2xl uppercase flex items-center gap-2 text-rose-600"><UserSearch className="h-6 w-6" /> Tribe Member Records</CardTitle>
         ...
         <CardContent className="px-0 space-y-8">
           <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4">
              <Input placeholder={recordSearchMode === 'id' ? "Enter ID..." : "Enter Username..."} value={recordSearchId} onChange={(e) => setRecordSearchId(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(recordSearchMode, recordSearchId, setTargetUserForRecord, setIsSearchingRecord)} className="h-14 rounded-2xl border-2" />
              <Button onClick={() => handleGenericSearch(recordSearchMode, recordSearchId, setTargetUserForRecord, setIsSearchingRecord)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingRecord}>Audit Identity</Button>
            </div>
           </div>
            ...
            <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
             <div className="p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 flex items-center justify-between">
               <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold uppercase text-slate-900">{targetUserForRecord.username}</h3>
                  <div className="flex flex-col gap-1 mt-1">
                   <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Account: {targetUserForRecord.accountNumber}</span>
                  </div>
                </div>
               </div>
               <div className="text-right space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Account Established</p>
                <p className="font-bold text-slate-900 uppercase ">{targetUserForRecord.createdAt?.toDate() ? format(targetUserForRecord.createdAt.toDate(), 'PPP') : 'Stardust'}</p>
               </div>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="p-6 bg-blue-50 rounded-3xl border-2 border-blue-100 space-y-2"><div className="flex items-center gap-2 text-blue-600 mb-2"><Wallet className="h-4 w-4" /><span className="text-[10px] font-bold uppercase tracking-wider">Wallet Balance</span></div><div className="flex items-center gap-2 text-2xl font-bold text-blue-900 "><GoldCoinIcon className="h-6 w-6" />{targetUserForRecord.wallet?.coins.toLocaleString() || 0}</div></div>
               <div className="p-6 bg-cyan-50 rounded-3xl border-2 border-cyan-100 space-y-2"><div className="flex items-center gap-2 text-cyan-600 mb-2"><Sparkles className="h-4 w-4" /><span className="text-[10px] font-bold uppercase tracking-wider">Diamonds Received</span></div><div className="flex items-center gap-2 text-2xl font-bold text-cyan-900 "><Activity className="h-6 w-6" />{targetUserForRecord.wallet?.diamonds.toLocaleString() || 0}</div></div>
               <div className="p-6 bg-purple-50 rounded-3xl border-2 border-purple-100 space-y-2"><div className="flex items-center gap-2 text-purple-600 mb-2"><History className="h-4 w-4" /><span className="text-[10px] font-bold uppercase tracking-wider">Total Spend Record</span></div><div className="flex items-center gap-2 text-2xl font-bold text-purple-900 "><BarChart3 className="h-6 w-6" />{targetUserForRecord.wallet?.totalSpent.toLocaleString() || 0}</div></div>
             </div>

             <div className="p-8 bg-red-50 rounded-3xl border-2 border-red-100 flex flex-col items-center gap-6">
               <div className="text-center space-y-2">
                <h4 className="text-xl font-bold uppercase text-red-600">Supreme Wallet Purge</h4>
                <p className="textxs font-body text-red-800/60 max-w-sm">DANGER: This protocol will PERMANENTLY reset this member's Coins, Diamonds, and Spend history to zero. Use only for catastrophic protocol violations.</p>
               </div>
               <Button onClick={handleResetWallet} disabled={isResettingWallet} variant="destructive" className="h-16 px-12 rounded-2xl font-bold uppercase text-lg shadow-xl shadow-red-500/20 active:scale-95 transition-all">
        ...
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         ...
         <div className="flex flex-col gap-4">
           ...
           <div className="flex gap-4">
            ...
            <Button onClick={() => handleGenericSearch(centerSearchMode, centerSearchId, setTargetUserForCenter, setIsSearchingCenter)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingCenter}>Find Identity</Button>
           </div>
         </div>
         {targetUserForCenter && (
          <div className="mt-10 p-8 border-2 rounded-3xl space-y-8 animate-in slide-in-from-bottom-4 bg-slate-50/20">
            <div className="flex items-center justify-between border-b pb-6">
             ...
             <div className="text-right">
               ...
               {targetUserForCenter.tags?.some((t: string) => ['Seller', 'Seller center', 'Coin Seller'].includes(t)) ? (
                <Badge className="bg-green-500 text-white font-bold uppercase text-[10px] py-1 px-3">Center Active</Badge>
               ) : (
                <Badge className="bg-slate-200 text-slate-400 font-bold uppercase text-[10px] py-1 px-3 shadow-none">Center Inactive</Badge>
               )}
             </div>
            </div>
            <Button onClick={handleToggleSellerCenter} className={cn("w-full h-16 rounded-xl font-bold uppercase text-xl shadow-xl transition-all", targetUserForCenter.tags?.some((t: string) => ['Seller', 'Seller center', 'Coin Seller'].includes(t)) ? "bg-red-50 text-red-600 border-2 border-red-100 hover:bg-red-100" : "bg-indigo-600 text-white hover:bg-indigo-700")}>
        ...
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         ...
         <div className="flex flex-col gap-4">
           ...
           <div className="flex gap-4">
            ...
            <Button onClick={() => handleGenericSearch(banSearchMode, banSearchId, setTargetUserForBan, setIsSearchingBan)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingBan}>Locate Identity</Button>
           </div>
         </div>
          ...
          <div className="mt-10 p-8 border-2 rounded-3xl space-y-8 animate-in slide-in-from-bottom-4 bg-slate-50/20">
            <div className="flex items-center justify-between border-b pb-6">
             <div className="flex items-center gap-4">
               ...
               <div>
                <p className="font-bold uppercase text-xl tracking-tight text-slate-900">{targetUserForBan.username}</p>
                <span className="text-[10px] font-bold text-slate-400 uppercase">Account: {targetUserForBan.accountNumber}</span>
               </div>
             </div>
             <div className="text-right">
               <p className="text-[10px] font-bold uppercase text-gray-400 mb-1">Authorization State</p>
               {targetUserForBan.banStatus?.isBanned ? (
                <Badge className="bg-red-600 text-white font-bold uppercase text-[10px] py-1 px-3">BANNED</Badge>
               ) : (
                <Badge className="bg-green-500 text-white font-bold uppercase text-[10px] py-1 px-3 shadow-none">AUTHORIZED</Badge>
               )}
             </div>
            </div>
            ...
            {!targetUserForBan.banStatus?.isBanned ? (
             <div className="space-y-6">
              <div className="grid gap-6">
                <div className="flex items-center justify-between px-1">
                 <p className="text-[10px] font-bold uppercase text-gray-400">Temporal Exclusion Offset</p>
                 <button onClick={() => setIsPermanentBan(!isPermanentBan)} className={cn("text-[8px] font-bold uppercase px-2 py-0.5 rounded-md transition-all", isPermanentBan ? "bg-red-600 text-white" : "bg-slate-100 text-slate-400")}>
                ...
                {!isPermanentBan ? (
                 <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold uppercase text-gray-400 text-center">Days</p>
                    <Input type="text" inputMode="numeric" value={banDays} onChange={(e) => setBanDays(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-2 text-xl font-bold text-center" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold uppercase text-gray-400 text-center">Hours</p>
                    <Input type="text" inputMode="numeric" value={banHours} onChange={(e) => setBanHours(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-2 text-xl font-bold text-center" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold uppercase text-gray-400 text-center">Minutes</p>
                    <Input type="text" inputMode="numeric" value={banMinutes} onChange={(e) => setBanMinutes(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-2 text-xl font-bold text-center" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold uppercase text-gray-400 text-center">Seconds</p>
                    <Input type="text" inputMode="numeric" value={banSeconds} onChange={(e) => setBanSeconds(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-2 text-xl font-bold text-center" />
                  </div>
                 </div>
                ) : (
                 <div className="h-14 rounded-2xl border-2 border-red-100 bg-red-50 flex items-center justify-center gap-2">
                  <Gavel className="h-5 w-5 text-red-600" />
                  <span className="font-bold uppercase text-red-600">Infinite Exclusion Protocol</span>
                 </div>
                )}
              </div>
              <Button onClick={handleBanUser} disabled={isBanning} className="w-full h-16 rounded-xl bg-red-600 text-white font-bold uppercase text-xl shadow-xl hover:bg-red-700 active:scale-95 transition-all">
             ...
             <div className="space-y-6">
              <div className="bg-red-50 p-6 rounded-2xl border-2 border-red-100 space-y-2">
                <p className="text-[10px] font-bold uppercase text-red-600">Ban Active Until</p>
              ...
              <Button onClick={handleUnbanUser} disabled={isBanning} className="w-full h-16 rounded-xl bg-green-600 text-white font-bold uppercase text-xl shadow-xl hover:bg-green-700 active:scale-95 transition-all">
      ...
      <TabsContent value="themes" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0"><CardTitle className="text-2xl uppercase flex items-center gap-2 text-rose-500"><Palette className="h-6 w-6" /> Theme Hub</CardTitle></CardHeader>
         <CardContent className="px-0 space-y-8">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-3xl border border-slate-100">
            <div className="space-y-4">
              <Input placeholder="Theme Name..." value={newThemeName} onChange={(e) => setNewThemeName(e.target.value)} className="h-14 rounded-2xl border-2 bg-white text-lg font-bold shadow-sm" />
              <div className="grid grid-cols-2 gap-2">
               <div className="space-y-1">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Price (Coins)</Label>
                <Input type="number" value={newThemePrice} onChange={(e) => setNewThemePrice(e.target.value)} className="h-12 rounded-xl border-2" />
               </div>
               <div className="space-y-1">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Duration (Days)</Label>
              ...
              <Select value={newThemeCategory} onValueChange={(val: any) => setNewThemeCategory(val)}>
               <SelectTrigger className="h-14 rounded-2xl border-2 bg-white font-bold "><SelectValue placeholder="Category" /></SelectTrigger>
               <SelectContent className="bg-white rounded-2xl font-bold ">
            ...
            <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-3xl bg-white p-6 group">
              <button onClick={() => themeFileInputRef.current?.click()} disabled={isUploadingTheme} className="flex flex-col items-center gap-3 active:scale-95 transition-transform">
               ...
               <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Upload visual sync</span>
              </button>
            ...
            {customThemes?.map((theme) => (
             <Card key={theme.id} className="rounded-2xl overflow-hidden border-2 shadow-sm group relative">
               ...
               <div className="p-3 text-center border-t">
                <p className="text-[10px] font-bold uppercase truncate">{theme.name}</p>
      ...
      <TabsContent value="banners" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0 flex flex-row items-center justify-between">
           <div>
            <CardTitle className="text-2xl uppercase flex items-center gap-2 text-blue-600"><ImageIcon className="h-6 w-6" /> Global Banners</CardTitle>
            <CardDescription>Manage unlimited event banners. Set titles, subtitles, and icons.</CardDescription>
           </div>
           <Button onClick={handleAddBanner} className="bg-primary text-black font-bold uppercase rounded-xl h-12 shadow-lg shadow-primary/20">
         ...
         <CardContent className="px-0 space-y-8">
           <div className="grid grid-cols-1 gap-8">
            {(bannerConfig?.slides || DEFAULT_SLIDES).map((slide: any, idx: number) => (
             <div key={idx} className="p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 flex flex-col md:flex-row gap-8 group relative transition-all hover:border-blue-100">
              ...
              <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-2">
                 <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Banner Title</Label>
                 <Input 
                  value={slide.title} 
                ...
                <div className="space-y-2">
                 <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Subtitle Frequency</Label>
                 <Input 
                  value={slide.subtitle} 
                  onChange={(e) => handleUpdateBannerMeta(idx, 'subtitle', e.target.value)}
                  placeholder="e.g. Global Frequency Sync"
                  className="h-14 rounded-2xl border-2 font-body "
                 />
                </div>
                <div className="space-y-2">
                 <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Icon Signature</Label>
                ...
                <div className="flex items-end">
                 <Button 
                  ...
                  className="w-full h-14 rounded-2xl uppercase font-bold text-sm shadow-lg shadow-red-500/10"
                 >
      ...
      <TabsContent value="broadcaster" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-white p-8">
         <CardHeader className="px-0"><CardTitle className="text-2xl uppercase flex items-center gap-2 text-slate-900"><Megaphone className="h-6 w-6 text-primary" /> Global Broadcaster</CardTitle></CardHeader>
         <CardContent className="px-0 space-y-6">
           <div className="space-y-4">
            <Input value={broadcastTitle} onChange={(e) => setBroadcastTitle(e.target.value)} className="h-14 rounded-2xl border-2 text-lg font-bold " />
            <Textarea placeholder="Type official message..." value={broadcastContent} onChange={(e) => setBroadcastContent(e.target.value)} className="h-40 rounded-3xl border-2 p-6 text-base font-body resize-none" />
           </div>
           <Button onClick={handleSystemBroadcast} disabled={isBroadcasting || !broadcastContent.trim()} className="w-full h-16 rounded-xl bg-slate-900 text-white font-bold uppercase text-xl shadow-xl">
        ...
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         ...
         <div className="flex flex-col gap-4">
           ...
           <div className="flex gap-4">
            ...
            <Button onClick={() => handleGenericSearch(dmSearchMode, dmSearchId, setTargetUserForDm, setIsSearchingDm)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingDm}>Find Identity</Button>
           </div>
         </div>
         {targetUserForDm && (
          <div className="mt-10 p-8 border-2 rounded-3xl space-y-8 animate-in slide-in-from-bottom-4 bg-slate-50/20">
            <div className="flex items-center gap-4 border-b pb-6">
             ...
             <div>
               <p className="font-bold uppercase text-xl tracking-tight text-slate-900">{targetUserForDm.username}</p>
               <span className="text-[10px] font-bold text-slate-400 uppercase">Account: {targetUserForDm.accountNumber}</span>
             </div>
            </div>
            <div className="space-y-4">
             <Input value={dmTitle} onChange={(e) => setDmTitle(e.target.value)} className="h-14 rounded-2xl border-2 text-lg font-bold " />
             <Textarea placeholder="Type private system message..." value={dmContent} onChange={(e) => setDmContent(e.target.value)} className="h-40 rounded-3xl border-2 p-6 text-base font-body resize-none" />
            </div>
            <Button onClick={() => handleDirectMessage()} disabled={isSendingDm || !dmContent.trim()} className="w-full h-16 rounded-xl bg-indigo-600 text-white font-bold uppercase text-xl shadow-xl">
      ...
      <TabsContent value="games" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl bg-gradient-to-br from-primary/10 to-transparent">
         <CardHeader><CardTitle className="text-2xl uppercase flex items-center gap-2 text-primary"><Gamepad2 className="h-6 w-6" /> Game Identity Sync</CardTitle><CardDescription>Synchronize visuals for the 3D Tribe Arena.</CardDescription></CardHeader>
           ...
           {gamesList.map((game) => (
            <Card key={game.slug} className="rounded-3xl overflow-hidden border-2 shadow-sm bg-white flex flex-col">
             ...
             <div className="p-4 flex-1 flex flex-col gap-3">
               <CardTitle className="text-sm font-bold uppercase text-slate-900 text-center">{game.title}</CardTitle>
               <div className="flex flex-col gap-2">
                <Button onClick={() => handleGameDPUploadClick(game)} className="w-full h-10 rounded-2xl bg-primary text-white font-bold uppercase text-[10px] shadow-lg" disabled={isUploadingGameDP}>
                  <Camera className="h-3 w-3 mr-2" /> Sync Cover (DP)
                </Button>
                <Button onClick={() => handleGameBGUploadClick(game)} className="w-full h-10 rounded-2xl bg-indigo-600 text-white font-bold uppercase text-[10px] shadow-lg" disabled={isUploadingGameBG}>
                  <Monitor className="h-3 w-3 mr-2" /> Sync Theme (BG)
                </Button>
               </div>
               {game.backgroundUrl && (
                <div className="mt-2 text-center">
                 <p className="text-[8px] font-bold text-green-600 uppercase tracking-wider flex items-center justify-center gap-1">
      ...
      <TabsContent value="tags" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         <CardHeader className="px-0"><CardTitle className="text-2xl uppercase flex items-center gap-2 text-slate-900"><BadgeCheck className="h-6 w-6 text-primary" /> Assign Official Tags</CardTitle></CardHeader>
         <div className="flex flex-col gap-4">
           ...
           <div className="flex gap-4">
            ...
            <Button onClick={() => handleGenericSearch(tagSearchMode, tagSearchId, setTargetUserForTags, setIsSearchingTag)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingTag}>Find Identity</Button>
           </div>
         </div>
         {targetUserForTags && (
          <div className="mt-10 p-6 border-2 rounded-2xl flex flex-col gap-8 animate-in slide-in-from-bottom-4 bg-slate-50/30">
            <div className="flex items-center justify-between">
             <div className="flex items-center gap-4">
               <Avatar className="h-16 w-16 border-2 border-white shadow-xl"><AvatarImage src={targetUserForTags.avatarUrl || undefined}/></Avatar>
               <div>
                <p className="font-bold uppercase text-xl tracking-tight text-slate-900">{targetUserForTags.username}</p>
                <span className="text-[10px] font-bold text-slate-400 uppercase">Account: {targetUserForTags.accountNumber}</span>
               </div>
             </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
             {ELITE_TAGS.map(tag => (
              <Button key={tag.id} variant={targetUserForTags.tags?.includes(tag.id) ? 'default' : 'outline'} className="h-16 rounded-2xl font-bold uppercase text-xs transition-all border-2" onClick={() => toggleUserRole(targetUserForTags.id, tag.id, targetUserForTags.tags)}>{tag.label}</Button>
             ))}
            </div>
            <Button variant="ghost" onClick={() => handleRemoveAllTags(targetUserForTags.id)} className="text-red-500 text-[10px] font-bold uppercase">Purge All Authority</Button>
          </div>
         )}
        </Card>
      </TabsContent>
      ...
      <TabsContent value="rewards" className="m-0 space-y-6">
        <Card className="rounded-3xl border-none shadow-xl p-8 bg-white">
         <CardHeader className="px-0"><CardTitle className="text-2xl uppercase flex items-center gap-2 text-slate-900"><Gift className="h-6 w-6 text-primary" /> Sovereign Dispatch Center</CardTitle></CardHeader>
         <div className="flex flex-col gap-4">
           ...
           <div className="flex gap-4">
            ...
            <Button onClick={() => handleGenericSearch(rewardSearchMode, rewardSearchId, setTargetUserForRewards, setIsSearchingRewards)} className="h-14 px-8 rounded-2xl bg-black text-white font-bold uppercase " disabled={isSearchingRewards}>Find Identity</Button>
           </div>
         </div>
         {targetUserForRewards && (
          <div className="mt-10 p-8 border-2 rounded-3xl space-y-10 animate-in slide-in-from-bottom-4 bg-slate-50/20">
            <div className="flex items-center gap-4 border-b pb-6">
             ...
             <div>
               <p className="font-bold uppercase text-xl tracking-tight text-slate-900">{targetUserForRewards.username}</p>
               <span className="text-[10px] font-bold text-slate-400 uppercase">Account: {targetUserForRewards.accountNumber}</span>
             </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
             <div className="space-y-4"><div className="flex items-center gap-2 text-primary"><GoldCoinIcon className="h-5 w-5" /><h4 className="font-bold uppercase text-sm">Coin Frequency</h4></div><div className="flex gap-2"><Input placeholder="Volume..." value={coinDispatchAmount} onChange={(e) => setCoinDispatchAmount(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-2 text-lg font-bold " /><Button onClick={handleDispatchCoins} disabled={isDispatching} className="h-14 px-8 bg-primary text-white rounded-2xl"><Send className="h-5 w-5" /></Button></div></div>
             <div className="space-y-4"><div className="flex items-center gap-2 text-purple-500"><Star className="h-5 w-5" /><h4 className="font-bold uppercase text-sm">Sync Elite Frames</h4></div><div className="flex flex-wrap gap-2">{DISPATCH_ASSETS.frames.map(frame => (<Button key={frame.id} variant="outline" size="sm" onClick={() => handleDispatchItem(frame.id as any, 'ownedItems')} className="h-10 text-[8px] font-bold uppercase rounded-xl">{frame.name}</Button>))}</div></div>
            </div>
          </div>
         )}
        </Card>
