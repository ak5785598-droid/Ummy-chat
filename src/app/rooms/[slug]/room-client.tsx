...
export function RoomClient({ room }: { room: Room }) {
  ...
  const { remoteStreams } = useWebRTC(room.id, isInSeat, currentUserParticipant?.isMuted ?? true, null);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (isChatMuted && !canManageRoom) return;
  ...
  return (
    <div className="relative flex flex-col h-full bg-black overflow-hidden text-white font-headline">
      ...
      <div className="absolute inset-0 z-0">
        <Image src={currentTheme.url} alt="Background" fill unoptimized className="object-cover opacity-60" priority />
      ...
      <header className="relative z-50 flex items-center justify-between p-4 pt-4">
        <div className="flex items-center gap-3 ml-12">
          <div className="relative">
            ...
            <div className="absolute -bottom-2 -left-1 flex items-center gap-0.5 bg-black/60 backdrop-blur-md px-1.5 py-0.5 rounded-full border border-white/10 z-20 shadow-lg">
               ...
               <span className="text-[8px] font-black text-yellow-400 leading-none">{room.stats?.totalGifts?.toLocaleString() || 0}</span>
            </div>
          </div>
          <div className="flex flex-col">
             <div className="flex items-center gap-2">
                <h1 className="font-black text-[15px] uppercase tracking-tighter text-white leading-none">{room.title}</h1>
                <button onClick={() => {}} className="h-6 w-6 rounded-full flex items-center justify-center bg-[#00E676] active:scale-90 shadow-lg">
                   <Plus className="h-4 w-4 text-white" strokeWidth={4} />
                </button>
             </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {canManageRoom && <RoomSettingsDialog room={room} trigger={<button className="p-2 bg-white/10 rounded-full active:scale-95 transition-transform"><Hexagon className="h-5 w-5" /></button>} />}
      ...
      <main className="relative z-10 flex-1 flex flex-col pt-0 overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-start gap-2 pt-4 pb-40 overflow-y-auto no-scrollbar">
           ...
           {/* Announcement Positioned Beneath Seats */}
        ...
        {/* Global Glossy Card Style for all interactions */}
                 ...
                 {firestoreMessages?.map((msg: any) => (
                   ...
                   >
                      ...
                      <div className="flex flex-col">
                        ...
                        <p className="text-[11px] font-bold text-white leading-tight break-all">
                          {msg.type === 'entrance' ? 'entered the room' : (msg.content || msg.text)}
                        </p>
                      </div>
                   </div>
                 ))}
      ...
      <footer className="relative z-50 px-4 pb-10 flex items-center justify-between pt-4">
        <div className="flex items-center ml-4">
           <button onClick={() => setShowInput(true)} className={cn("backdrop-blur-xl rounded-full h-12 w-12 flex items-center justify-center cursor-pointer transition-all shrink-0 shadow-lg", isChatMuted && !canManageRoom ? "bg-red-500/20 text-red-400 border border-red-500/20" : "bg-white/10 text-white")}>
        ...
        <div className="flex items-center gap-2">
           ...
           <button onClick={() => setIsEmojiPickerOpen(true)} className="p-2 bg-white/10 rounded-full active:scale-90 transition-transform shadow-md border border-white/5"><SmilePlus className="h-5 w-5 text-white" /></button>
           <button onClick={() => setIsMessagesOpen(true)} className="p-2 bg-white/10 rounded-full active:scale-90 transition-transform shadow-md border border-white/5"><Mail className="h-5 w-5 text-white" /></button>
           <button onClick={() => setIsRoomPlayOpen(true)} className="p-2 bg-white/10 rounded-full active:scale-90 transition-transform shadow-md border border-white/5"><LayoutGrid className="h-5 w-5 text-white" /></button>
        </div>
      </footer>
      ...
      <RoomPlayDialog open={isRoomPlayOpen} onOpenChange={setIsRoomPlayOpen} participants={participants} roomId={room.id} room={room} isMutedLocal={isMutedLocal} setIsMutedLocal={setIsMutedLocal} onOpenGames={() => setIsRoomGamesOpen(true)} />
