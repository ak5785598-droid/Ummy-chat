 ...
 * Synchronized with the high-fidelity [id] path protocol.
 */
export default function RoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const banDocRef = useMemoFirebase(() => {
    if (!firestore || !id || isUserLoading || !currentUser) return null;
    return doc(firestore, 'chatRooms', id, 'bans', currentUser.uid);
  }, [firestore, id, isUserLoading, currentUser]);
  ...
  const activeRoom: Room | null = useMemo(() => {
    if (firestoreRoom) {
      return {
        ...
        announcement: firestoreRoom.announcement || "Welcome to the tribe!",
