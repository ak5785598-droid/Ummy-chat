        ...
        {headerTab === 'recommend' ? (
          <>
            <section className="px-6 grid grid-cols-3 gap-4 mb-8">
              <button onClick={() => router.push('/leaderboard?type=rich')} className="group relative aspect-square translate-x-1 rounded-[1.5rem] bg-gradient-to-br from-[#ffd700] via-[#ff9800] to-[#f57c00] border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 <div className="absolute inset-0 bg-white/20 -skew-x-[30deg] -translate-x-[200%] animate-shine" />
                 <span className="absolute top-2 left-3 text-white font-black uppercase text-[8px] tracking-widest opacity-90">{t.profile.level}</span>
              ...
              <button onClick={() => router.push('/leaderboard?type=games')} className="group relative aspect-square rounded-[1.5rem] bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-700 border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 ...
                 <span className="absolute top-2 left-3 text-white font-black uppercase text-[8px] tracking-widest opacity-90">{t.nav.games}</span>
                 <div className="relative z-10 group-hover:scale-110 transition-transform">
                    <Gamepad2 className="h-12 w-12 text-white fill-indigo-200 drop-shadow-[0_0_15px_#ffffffcc]" />
                 </div>
              </button>
              <button onClick={() => router.push('/cp-challenge')} className="group relative aspect-square -translate-x-1 rounded-[1.5rem] bg-gradient-to-br from-[#ff4d4d] via-[#f43f5e] to-[#be123c] border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 <div className="absolute inset-0 bg-white/20 -skew-x-[30deg] -translate-x-[200%] animate-shine delay-700" />
                 <span className="absolute top-2 left-3 text-white font-black uppercase text-[8px] tracking-widest opacity-90">{t.profile.cp}</span>
                 <div className="relative z-10 group-hover:scale-110 transition-transform">
            ...
            <div className="px-6 mb-6">
              <div className="bg-gradient-to-r from-[#9C27B0] via-[#E91E63] to-[#9C27B0] h-12 rounded-full shadow-2xl border-2 border-white/40 flex items-center justify-between px-6 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all">
                ...
                <div className="flex items-center gap-2 relative z-10">
                   ...
                   <span className="text-white font-black uppercase italic text-xs tracking-widest drop-shadow-md">{t.home.topRooms}</span>
                </div>
            ...
            <main className="px-4 flex-1">
              {isRoomsLoading && !roomsData ? (
                ...
                <div className="py-20 text-center space-y-4 opacity-40">
                   ...
                   <p className="font-black uppercase italic text-xs tracking-widest">{t.home.noActive}</p>
                </div>
              )}
            </main>
          </>
          ...
          <main className="px-6 flex-1 animate-in slide-in-from-right-4 duration-500">
             <section className="mb-10">
                <h3 className="text-xl font-black uppercase italic tracking-tighter text-slate-900 mb-6 flex items-center gap-2">
                   <Zap className="h-5 w-5 text-primary fill-current" /> {t.profile.id} {t.home.mine}
                </h3>
                {myRoom ? (
                  ...
                  >
                     ...
                     <span className="text-xs font-black uppercase tracking-widest text-slate-400">{t.home.launch}</span>
                  </button>
                )}
             </section>
             ...
             <section>
                <h3 className="text-xl font-black uppercase italic tracking-tighter text-slate-900 mb-6 flex items-center gap-2">
                   <Heart className="h-5 w-5 text-pink-500 fill-current" /> {t.profile.follow}
                </h3>
                {isFollowedLoading ? (
