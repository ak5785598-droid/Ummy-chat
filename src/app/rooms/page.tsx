        ...
        {headerTab === 'recommend' ? (
          <>
            <section className="px-5 grid grid-cols-3 gap-3 mb-4">
              <button onClick={() => router.push('/leaderboard?type=rich')} className="group relative aspect-square rounded-[1.25rem] bg-gradient-to-br from-[#ffd700] via-[#ff9800] to-[#f57c00] border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 <div className="absolute inset-0 bg-white/20 -skew-x-[30deg] -translate-x-[200%] animate-shine" />
                 <span className="absolute top-1.5 left-2 text-white font-black uppercase text-[7px] tracking-widest opacity-90">{t.profile.level}</span>
              ...
              <button onClick={() => router.push('/leaderboard?type=games')} className="group relative aspect-square rounded-[1.25rem] bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-700 border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 ...
                 <span className="absolute top-1.5 left-2 text-white font-black uppercase text-[7px] tracking-widest opacity-90">{t.nav.games}</span>
                 <div className="relative z-10 group-hover:scale-110 transition-transform">
              ...
              <button onClick={() => router.push('/cp-challenge')} className="group relative aspect-square rounded-[1.25rem] bg-gradient-to-br from-[#ff4d4d] via-[#f43f5e] to-[#be123c] border-2 border-white/30 shadow-xl overflow-hidden active:scale-95 transition-all flex flex-col items-center justify-center p-2">
                 ...
                 <span className="absolute top-1.5 left-2 text-white font-black uppercase text-[7px] tracking-widest opacity-90">{t.profile.cp}</span>
                 <div className="relative z-10 group-hover:scale-110 transition-transform">
            ...
            <div className="px-5 mb-4">
              <div className="bg-gradient-to-r from-[#9C27B0] via-[#E91E63] to-[#9C27B0] h-10 rounded-full shadow-lg border-2 border-white/40 flex items-center justify-between px-5 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all">
                ...
                <div className="flex items-center gap-2 relative z-10">
                   ...
                   <span className="text-white font-black uppercase italic text-[10px] tracking-widest drop-shadow-md">{t.home.topRooms}</span>
                </div>
            ...
            <main className="px-3.5 flex-1">
              {isRoomsLoading && !roomsData ? (
                        ...
                        {displaySlides.map((slide: any, idx: number) => {
                          ...
                          return (
                            <CarouselItem key={idx}>
                              <div className={cn("h-20 w-full rounded-[1.75rem] bg-gradient-to-br p-3.5 flex flex-col justify-center relative overflow-hidden shadow-lg border-2 border-white/20 active:scale-[0.98] transition-all group", slide.color || 'from-purple-600 to-indigo-600')}>
                                 {slide.imageUrl && (
                                   <Image 
                                     src={slide.imageUrl} 
                                     alt="" 
                                 ...
                                 <div className="relative z-10">
                                    <div className="flex items-center gap-1.5 mb-0.5">
                                       <Icon className="h-3.5 w-3.5 text-white animate-pulse" />
                                       <h3 className="text-base font-black uppercase italic tracking-tighter text-white drop-shadow-md">{slide.title}</h3>
                                    </div>
                                    <p className="text-[9px] font-bold text-white/70 uppercase tracking-widest">{slide.subtitle || slide.sub}</p>
                                 </div>
                ...
                <div className="py-16 text-center space-y-3 opacity-40">
                   ...
                   <p className="font-black uppercase italic text-[10px] tracking-widest">{t.home.noActive}</p>
                </div>
              )}
            </main>
          </>
          ...
          <main className="px-5 flex-1 animate-in slide-in-from-right-4 duration-500">
             <section className="mb-8">
                <h3 className="text-lg font-black uppercase italic tracking-tighter text-slate-900 mb-4 flex items-center gap-2">
                   <Zap className="h-4 w-4 text-primary fill-current" /> {t.profile.id} {t.home.mine}
                </h3>
                {myRoom ? (
                  ...
                  >
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.home.launch}</span>
                  </button>
                )}
             </section>
             ...
             <section>
                <h3 className="text-lg font-black uppercase italic tracking-tighter text-slate-900 mb-4 flex items-center gap-2">
                   <Heart className="h-4 w-4 text-pink-500 fill-current" /> {t.profile.follow}
                </h3>
                {isFollowedLoading ? (
