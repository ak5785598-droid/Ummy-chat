...
export default function SvipClubPage() {
  ...
  const SvipBadge = ({ level }: { level: number }) => (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 relative z-10">
         ...
         <span className="text-xs font-black text-white uppercase italic tracking-widest drop-shadow-md">SVIP {level}</span>
      </div>
    </div>
  );
  ...
  return (
    <AppLayout hideSidebarOnMobile hideBottomNav>
      <div className="min-h-full bg-[#0a0514] text-white font-headline relative flex flex-col pb-20 overflow-x-hidden animate-in fade-in duration-700">
        ...
        <header className="px-6 pt-10 pb-6 flex items-center justify-between sticky top-0 bg-[#0a0514]/80 backdrop-blur-md z-50">
           ...
           <h1 className="text-2xl font-black uppercase italic tracking-tighter">SVIP Club</h1>
           <div className="w-10" />
        </header>
        ...
        <main className="flex-1 p-6 space-y-10">
           ...
           <div className="flex flex-col items-center gap-8 relative py-10">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                 ...
                 <div className="w-48 h-48 bg-[#38bdf8]/10 blur-[80px] rounded-full animate-pulse" style={{ animationDelay: '1s' }} />
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center text-center gap-6">
                 ...
                 <div className="space-y-2">
                    <h2 className="text-3xl font-black uppercase italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#4ade80] to-[#38bdf8]">
                       Ringneck Eagle
                    </h2>
           ...
           <div className="flex items-center gap-3 overflow-x-auto no-scrollbar py-2">
              {[1, 2, 3, 4, 5, 6, 7].map(lvl => (
                <button 
                  ...
                  className={cn(
                    "shrink-0 h-14 w-14 rounded-2xl border-2 flex items-center justify-center font-black transition-all active:scale-90",
                    selectedLevel === lvl 
           ...
           <div className="space-y-6">
              <div className="flex items-center gap-3 ml-2">
                 ...
                 <h3 className="text-lg font-black uppercase tracking-tight">VIP Privileges</h3>
              </div>
                 ...
                 {[
                   ...
                   <div key={i} className={cn("p-5 rounded-3xl border-2 flex flex-col gap-3 relative overflow-hidden group", benefit.color)}>
                      ...
                      <div className="relative z-10">
                         <p className="text-xs font-black uppercase tracking-tight">{benefit.label}</p>
                         <p className="text-[8px] font-bold opacity-60 uppercase mt-0.5 tracking-widest">{benefit.desc}</p>
                      </div>
                   </div>
                 ))}
              </div>
        ...
        <footer className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent z-[100]">
           <Button className="w-full h-16 rounded-[1.5rem] bg-gradient-to-r from-[#4ade80] via-[#2dd4bf] to-[#38bdf8] text-white font-black uppercase italic text-xl shadow-xl shadow-green-900/40 active:scale-95 transition-all">
              Join SVIP Club
           </Button>
        </footer>
