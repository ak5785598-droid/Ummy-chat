    ...
    <AppLayout fullScreen>
      <div className={cn(
        ...
        <div className="relative z-10 flex flex-col h-full items-center justify-center">
          ...
          {/* MIDDLE AVATAR LINK LOBBY - NO CARD, CENTERED */}
          ...
          {/* Propose CP Button - Only show when no CP */}
          {!activeCp && (
            <button 
              onClick={() => setShowSearch(true)} 
              className="mt-8 px-6 py-2 bg-[#FF91B5] hover:bg-pink-400 transition text-white text-xs font-black rounded-full shadow-lg"
            >
              Propose CP
            </button>
          )}

          {/* Design Room Button - Only show when CP exists */}
          {activeCp && (
            <button 
              onClick={() => setIsEditMode(!isEditMode)}
              className="mt-8 px-6 py-2 bg-pink-400 hover:bg-pink-500 transition text-white text-xs font-black rounded-full shadow-lg flex items-center gap-2"
            >
              <Wrench className="h-3.5 w-3.5" /> {isEditMode ? 'Close Editor' : 'Design Room'}
            </button>
          )}
          ...
          {isEditMode && activeCp && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: 20 }}
              className="mt-6 mx-4 w-full max-w-md flex flex-col gap-3"
            >
              {/* CP Level Info */}
              <div className="flex items-center justify-between bg-white/90 backdrop-blur-xl rounded-2xl p-3 shadow-lg">
                <div className="flex flex-col">
                  <span className="text-xs font-black text-slate-800 uppercase tracking-tight">CP Level: {activeCp?.level || 1}</span>
                  <span className="text-[10px] text-pink-500 font-bold">Intimacy: {activeCp?.intimacyPoints || 0} pts</span>
                </div>
              </div>
              ...
              {/* --- EDITOR CONTROLS --- */}
              <div className="bg-white/95 backdrop-blur-xl border border-pink-100 rounded-2xl p-3 flex flex-col gap-2 shadow-xl">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">Editor Console</span>
                  <button 
