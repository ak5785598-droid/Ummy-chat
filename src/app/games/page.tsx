    ...
    <div className="relative z-10 space-y-10 max-w-7xl mx-auto p-6 pb-32">
     ...
     <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8">
      <div className="flex items-center gap-5">
       ...
       <div>
         <h1 className="font-sans text-5xl font-bold tracking-tight uppercase text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">
          3D Tribe Arena
         </h1>
         <p className="text-muted-foreground font-body text-lg opacity-60">High-fidelity 3D frequencies. Play together.</p>
       </div>
      </div>
      <div className="flex items-center gap-3">
        <div className="bg-white/5 backdrop-blur-xl px-6 py-2 rounded-2xl border border-white/10 flex items-center gap-3 shadow-2xl">
         ...
         <span className="text-sm font-bold tracking-tight">12,450 LIVE</span>
        </div>
      </div>
     </header>
     ...
     <section className="space-y-12">
       <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-sans text-2xl font-bold uppercase tracking-wider text-white/80">Select Dimension</h3>
        </div>
        {isSovereign && (
         <Badge className="bg-primary text-black font-bold uppercase animate-pulse">Identity Sync Active</Badge>
        )}
       </div>
         ...
         {activeGames.map((game) => (
          <div key={game.slug} className="group relative transition-all duration-500 transform-gpu preserve-3d hover:rotate-x-12 hover:rotate-y-6">
           <div className="block relative">
            <div className="absolute inset-0 bg-purple-600/20 rounded-3xl translate-z-[-20px] blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
            
            <Link href={`/games/${game.slug}`} className="relative aspect-square w-full rounded-3xl overflow-hidden border-2 border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 group-hover:border-purple-500/50 group-hover:shadow-[0_40px_80px_rgba(168,85,247,0.3)] group-hover:-translate-y-4 bg-gradient-to-br from-white/5 to-white/10 flex items-center justify-center">
              {game.coverUrl ? (
               <Image 
                key={game.coverUrl} 
                src={game.coverUrl} 
               ...
               <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-[8px] font-bold uppercase tracking-wider opacity-20 group-hover:opacity-100 transition-opacity">Loading character...</span>
               </div>
              )}
              ...
              <div className="absolute bottom-4 right-4 flex items-center gap-1.5 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10 shadow-xl">
               ...
               <span className="text-[10px] font-bold text-white/80">{(liveCounts[game.slug] || 100).toLocaleString()}</span>
              </div>
            </Link>
            
            <div className="mt-6 px-2 space-y-1 text-center translate-z-[30px]">
              <h4 className="font-bold text-sm uppercase truncate group-hover:text-purple-400 transition-colors tracking-tight drop-shadow-lg">{game.title}</h4>
              <div className="flex items-center justify-center gap-2">
               <div className="h-0.5 w-4 rounded-full bg-purple-500 group-hover:w-8 transition-all duration-500" />
               <span className="text-[9px] font-bold text-white/40 uppercase tracking-wider">3D Reality</span>
              </div>
            </div>
           </div>
          </div>
     ...
     <section className="pt-20">
       <div className="bg-gradient-to-br from-purple-900/40 to-blue-900/40 p-12 rounded-[4rem] border border-white/5 flex flex-col items-center text-center space-y-8 shadow-[0_0_100px_rgba(147,51,234,0.1)] relative overflow-hidden group">
        ...
        <div className="space-y-4 relative z-10">
          <h3 className="text-4xl font-bold uppercase tracking-tight">Become an Arena Legend</h3>
          <p className="text-white/60 max-w-md font-body text-lg ">Top tribe members earn exclusive 3D avatar frames and elite badges every 24 hours. Reset at 11:59:59 IST.</p>
        </div>
        <div className="flex gap-6 relative z-10">
          <Button variant="outline" className="rounded-full px-10 h-14 border-white/10 text-white font-bold uppercase text-xs hover:bg-white/5 shadow-2xl">Ranking History</Button>
          <Button className="rounded-full px-10 h-14 bg-purple-600 hover:bg-purple-500 text-white font-bold uppercase text-xs shadow-[0_10px_40px_rgba(147,51,234,0.4)] hover:scale-105 active:scale-95 transition-transform">Claim Rewards</Button>
        </div>
       </div>
     </section>
