...
const ANIMALS = [
 ...
 { id: 'tiger', emoji: '🐯', multiplier: 25, label: 'x25', pos: 'left', color: 'from-orange-400 to-orange-600', border: 'border-orange-400' },
 { id: 'lion', emoji: '🦁', multiplier: 45, label: 'x45', pos: 'top-left', color: 'from-yellow-400 to-red-600', border: 'border-yellow-400' },
];
...
export default function WildPartyPage() {
 ...
 if (isLaunching) {
  const gameDataAsAny = gameData as any;
  const loadingBg = gameDataAsAny?.loadingBackgroundUrl;
  return (
   <div 
    className="h-screen w-full bg-[#0a2e0a] flex flex-col items-center justify-center space-y-6 font-sans relative overflow-hidden"
   ...
   >
    ...
    <div className="relative z-10 flex flex-col items-center justify-center space-y-6">
      ...
      <h1 className="text-6xl font-bold text-yellow-500 uppercase tracking-tight drop-shadow-2xl">Wild Party</h1>
      <p className="text-white/40 uppercase tracking-wider text-[10px] animate-pulse">Entering the Jungle...</p>
    </div>
   </div>
  );
 }
  ...
  <AppLayout fullScreen>
   <div className="h-screen w-full bg-[#051a05] flex flex-col relative overflow-hidden font-sans text-white">
    ...
    <div className="absolute inset-0 z-0"> 
      {(gameData as any)?.backgroundUrl ? ( 
       <Image key={(gameData as any).backgroundUrl} src={(gameData as any).backgroundUrl} alt="Jungle Theme" fill className="object-cover opacity-60 animate-in fade-in duration-1000" unoptimized /> 
      ) : ( 
       <> 
    ...
    <div className="relative z-50 flex items-center justify-between p-4 pt-32"> 
      ...
      <h1 className="text-2xl font-bold text-yellow-500 uppercase tracking-tight drop-shadow-md">Wild Party</h1> 
      <div className="flex gap-2"> 
      ...
      <div className="bg-black/40 backdrop-blur-md rounded-full border border-white/10 p-1 flex items-center gap-2 overflow-x-auto no-scrollbar"> 
       {history.map((id, i) => ( 
        <div key={i} className="relative shrink-0"> 
          ...
          {i === 0 && <div className="absolute -top-1 -right-1 bg-red-500 text-[6px] font-bold px-1 rounded-full animate-pulse">NEW</div>} 
        </div> 
       ))} 
      </div> 
    </div> 
    ...
    <main className="flex-1 relative z-10 flex flex-col items-center justify-center py-6 px-4"> 
      <div className="relative w-full max-w-[360px] aspect-square flex items-center justify-center"> 
       ...
       <div className="relative z-20 w-32 h-24 bg-gradient-to-b from-[#2d1a12] to-[#1a0a2e] rounded-full shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col items-center justify-center border-4 border-[#b88a44] p-2 text-center overflow-hidden"> 
         <p className="text-[8px] font-bold uppercase text-yellow-500/60 leading-tight tracking-[0.2em] mb-1"> 
          {gameState === 'betting' ? 'BETTING' : 'SPINNING'} 
         </p> 
         <span className={cn( 
          "text-5xl font-bold tracking-tight transition-all duration-500", 
          gameState === 'betting' ? "text-white" : "text-yellow-400" 
         )}> 
           ...
           <div className="relative"> 
            <div className={cn( 
             ...
             isActive ? "border-white bg-gradient-to-br from-yellow-300 to-yellow-600 shadow-[0_0_30px_#facc15]" : `bg-gradient-to-br ${animal.color} ${animal.border}` 
              ...
              <span className="text-[8px] font-bold text-white/80 uppercase mt-1 leading-none tracking-wider relative z-10"> 
               {animal.label} 
              </span> 
            ...
            {betOnThis > 0 && ( 
             <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-40 animate-in zoom-in duration-300"> 
               <div className="bg-gradient-to-r from-[#d946ef] to-[#9333ea] px-4 py-1 rounded-full border border-white/40 shadow-xl flex items-center gap-1.5 whitespace-nowrap"> 
                <span className="text-[9px] font-bold text-white uppercase leading-none">Bet</span> 
                <div className="flex items-center gap-0.5"> 
                  <GoldCoinIcon className="h-3 w-3 text-yellow-400" /> 
                  <span className="text-[10px] font-bold text-white leading-none">{betOnThis.toLocaleString()}</span> 
                </div> 
               </div> 
             </div> 
            )} 
            ...
            {isActive && gameState === 'result' && ( 
             <div className="absolute inset-0 border-4 border-yellow-400 rounded-xl animate-ping" /> 
            )} 
           </div> 
         </button> 
        ); 
      ...
      <div className="max-w-md mx-auto space-y-4"> 
       <div className="flex items-center justify-between"> 
         <div className="flex items-center gap-2 bg-black/60 px-4 py-2 rounded-full border border-white/10 shadow-lg"> 
          ...
          <span className="text-lg font-bold text-yellow-500 ">{(userProfile?.wallet?.coins || 0).toLocaleString()}</span> 
         </div> 
       ...
       <div className="bg-[#2d1a12] p-3 rounded-3xl border-4 border-[#5d4037] shadow-2xl flex items-center justify-between gap-3 overflow-hidden relative"> 
         ...
         <button  
          ...
          className="relative z-10 bg-gradient-to-b from-orange-400 to-red-600 px-8 h-12 rounded-full font-bold uppercase text-xs shadow-xl active:scale-90 transition-all border-2 border-white/30" 
         > 
          Repeat 
         </button> 
       </div> 
