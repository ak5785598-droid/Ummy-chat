...
export default function LudoGamePage() {
 ...
 if (isLaunching) {
  return (
   <div className="h-screen w-full bg-[#0a1a4a] flex flex-col items-center justify-center space-y-6 font-sans">
    <Gamepad2 className="h-20 w-20 text-yellow-500 animate-bounce" />
    <h1 className="text-3xl font-bold text-white uppercase tracking-tight">Ludo Quick</h1>
   </div>
  );
 }
 ...
 return (
  <AppLayout fullScreen>
   <div className="h-screen w-full bg-black flex flex-col relative overflow-hidden font-sans" onClick={handleInteraction}>
    <CompactRoomView />
    ...
    <header className="relative z-40 p-3 pt-32 px-4 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent">
      ...
      <h1 className="text-lg font-bold text-white uppercase ">Ludo • Quick</h1>
      <div className="flex gap-2">
    ...
    <main className="flex-1 flex items-center justify-center p-2 relative z-10 pb-24">
      <div 
       className="relative w-[min(90vw,90vh)] aspect-square bg-[#7cb342] rounded-2xl p-3 shadow-2xl border-b-[8px] border-[#558b2f]"
       style={{ 
        display: 'grid', 
      ...
      >
       ...
       <div className="col-span-3 row-span-3 bg-gradient-to-b from-[#ffeb3b] to-[#fbc02d] rounded-xl flex items-center justify-center p-1"><span className="text-white text-xs font-bold uppercase">Play</span></div>
       <div className="col-span-6 row-span-3 grid grid-cols-6 grid-rows-3 gap-0.5">
         {Array.from({ length: 18 }).map((_, i) => <div key={i} className="rounded-sm bg-[#9ccc65] border border-black/5" />)}
       </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-sm px-4 z-[110]">
       <div className="bg-black/60 backdrop-blur-2xl p-3 rounded-full border border-white/10 flex items-center justify-between shadow-2xl">
         <div className="flex items-center gap-2 pl-2">
          ...
          <span className="text-[10px] font-bold text-white uppercase ">{userProfile?.username || 'Tribe'}</span>
         </div>
         ...
         <Badge className="bg-yellow-500 text-black font-bold uppercase text-[8px] px-3 py-1 rounded-lg">Quick Mode</Badge>
       </div>
      </div>
    </main>
   </div>
