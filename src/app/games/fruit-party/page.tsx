...
export default function FruitPartyPage() {
 ...
 if (isLaunching) {
  ...
  return (
   <div 
    className="h-screen w-full bg-[#311b92] flex flex-col items-center justify-center space-y-6 font-sans relative overflow-hidden"
   ...
   >
    ...
    <div className="relative z-10 flex flex-col items-center justify-center space-y-6">
      ...
      <h1 className="text-6xl font-bold text-yellow-400 uppercase tracking-tight drop-shadow-2xl">Fruit Party</h1>
      <p className="text-white/40 uppercase tracking-wider text-[10px] animate-pulse">Syncing Wheel...</p>
    </div>
   </div>
  );
 }
 ...
 return (
  <AppLayout fullScreen>
   <div className="h-[100dvh] w-full bg-[#58319d] flex flex-col relative overflow-hidden font-sans text-white">
    <CompactRoomView />
    ...
    <div className="relative z-50 flex items-center justify-between p-4 pt-32">
      ...
      <h1 className="text-xl font-bold text-white uppercase tracking-tight drop-shadow-md">Fruit Party</h1>
      <div className="flex gap-1">
    ...
    <main className="flex-1 relative z-10 flex flex-col items-center justify-center p-4">
      <div className="relative w-full max-w-[300px] aspect-square flex items-center justify-center">
       ...
       <div className="relative z-20 w-32 h-24 bg-[#4c1d95] rounded-full shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center border-[6px] border-[#7c3aed] p-4 text-center">
         <span className="text-6xl font-bold text-yellow-400 leading-none drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]">
          {gameState === 'betting' ? timeLeft : '🎲'}
         </span>
         <p className="text-[10px] font-bold uppercase text-white/60 tracking-wider mt-1">
       ...
       {ITEMS.map((item, idx) => (
        ...
        >
          <div className={cn(
           ...
           <span className="text-[7px] font-bold text-white/60 uppercase mt-1 leading-tight relative z-10">{item.label}</span>
          ...
          {myBets[item.id] > 0 && (
           <div className="mt-1 bg-yellow-400 text-black px-2 py-0.5 rounded-full font-bold text-[8px] shadow-lg animate-in zoom-in flex items-center gap-1">
    ...
    <footer className="relative z-50 p-4 pb-10 space-y-4 -translate-y-24">
      <div className="max-w-md mx-auto bg-[#7c3aed]/40 backdrop-blur-xl rounded-2xl p-4 border border-white/10 shadow-2xl">
       <div className="flex items-center justify-between mb-4 px-2">
         <div className="bg-gradient-to-r from-[#ffd54f] to-[#ffca28] px-4 py-1.5 rounded-xl flex items-center gap-2 shadow-lg">
          ...
          <span className="text-sm font-bold text-black">{(userProfile?.wallet?.coins || 0).toLocaleString()}</span>
         </div>
         <p className="text-[10px] font-bold uppercase text-white/60 ">Choose wager then food</p>
         <button onClick={handleRepeat} className="bg-white/10 px-6 py-1.5 rounded-xl font-bold uppercase text-xs hover:bg-white/20 active:scale-95 transition-all">Repeat</button>
       </div>
      ...
      <div className="max-w-md mx-auto bg-black/40 backdrop-blur-md rounded-full border border-white/5 p-2 px-6 flex items-center gap-4">
       <span className="text-[10px] font-bold uppercase tracking-tight text-white/40 shrink-0">Winning History</span>
       <div className="flex-1 flex gap-3 overflow-x-auto no-scrollbar">
         {history.map((id, i) => (
          <div key={i} className="relative shrink-0 flex flex-col items-center">
           ...
           {i === 0 && <span className="absolute -bottom-1 bg-pink-500 text-[6px] px-1 rounded-full font-bold animate-pulse">New</span>}
          </div>
         ))}
       </div>
      </div>
