...
export default function FruitSlotsPage() {
 ...
 if (isLaunching) {
  return (
   <div className="h-screen w-full bg-[#1a0a2e] flex flex-col items-center justify-center space-y-6 font-sans">
    <div className="text-8xl animate-bounce">🍋</div>
    <h1 className="text-6xl font-bold text-yellow-500 uppercase tracking-tight drop-shadow-2xl">Fruit Slots</h1>
    <p className="text-white/40 uppercase tracking-wider text-[10px] animate-pulse">Syncing Tribal Grid...</p>
   </div>
  );
 }
 ...
 return (
  <AppLayout fullScreen>
   <div className="h-[100dvh] w-full bg-[#311b92] flex flex-col relative overflow-hidden font-sans text-white select-none">
    <CompactRoomView />
    ...
    {gameState === 'result' && winners.length > 0 && (
     <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in zoom-in duration-500 p-6">
       <div className="relative mb-12 flex flex-col items-center gap-4">
        ...
        <h2 className="text-5xl font-bold text-white uppercase tracking-tight text-center drop-shadow-[0_0_20px_rgba(255,255,255,0.5)]">Big Win Sync</h2>
       </div>
       ...
       <div className="flex items-end justify-center gap-4 w-full max-w-lg">
        {winners.map((winner, idx) => (
         <div key={idx} className="flex flex-col items-center gap-2 animate-in slide-in-from-bottom-20 duration-700">
           ...
           <div className="bg-yellow-500/20 border-x-2 border-t-2 border-yellow-400 w-32 h-24 rounded-t-3xl flex flex-col items-center justify-center">
            <span className="text-3xl">🥇</span>
            <p className="text-[10px] font-bold text-white uppercase truncate px-2">{winner.name}</p>
            <p className="text-lg font-bold text-yellow-500">+{winner.win.toLocaleString()}</p>
           </div>
         </div>
        ))}
       </div>
    ...
    <header className="relative z-[110] flex items-center justify-between px-4 pt-32 pb-2">
      ...
      <div className="flex-1 mx-2">
       <div className="bg-[#3d2b1f] border-2 border-[#b88a44] rounded-full py-1 text-center shadow-lg px-4">
         <span className="text-[10px] font-bold uppercase text-[#f5e1a4]">Round 518 of Today</span>
       </div>
      </div>
    ...
    <main className="flex-1 flex flex-col items-center px-4 relative z-10 pb-4">
      <div className="w-full max-w-sm relative p-4 bg-gradient-to-b from-[#ffd700] via-[#b8860b] to-[#8b4513] rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-4 border-yellow-200/30">
       <div className="absolute inset-2 border-2 border-dashed border-white/20 rounded-2xl pointer-events-none" />
       
       <div className="grid grid-cols-3 gap-2 p-2 bg-[#1a0a2e] rounded-2xl relative overflow-hidden">
         ...
         {Array.from({ length: 9 }).map((_, idx) => {
          ...
          if (isCenter) {
           return (
            <div key={idx} className="aspect-square flex flex-col items-center justify-center bg-black/40 rounded-2xl border-2 border-yellow-500/20 shadow-inner">
             <span className="text-5xl font-bold text-yellow-400 tracking-tight drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]">
          ...
          return (
           ...
           >
            ...
            <span className="text-[10px] font-bold text-yellow-500 mt-1 uppercase ">{fruit.label}</span>
            ...
            {myBets[fruit.id] > 0 && (
             <div className="absolute inset-0 bg-yellow-500/10 flex items-center justify-center pointer-events-none">
               <div className="bg-black/60 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/20">
                <span className="text-[8px] font-bold text-yellow-400">{myBets[fruit.id].toLocaleString()}</span>
               </div>
             </div>
            )}
           </button>
      ...
      <div className="mt-6 w-full max-w-sm grid grid-cols-4 gap-3 px-2">
       {CHIPS.map(chip => (
        ...
        >
          <span className="text-sm font-bold text-white drop-shadow-md">{chip.label}</span>
        </button>
       ))}
      </div>
      ...
      <div className="mt-6 w-full max-w-sm flex gap-3">
       <div className="flex-1 bg-gradient-to-b from-[#1e3a8a] to-[#1e1b4b] rounded-2xl p-3 border-2 border-blue-400/20 shadow-xl relative group active:scale-95 transition-all cursor-pointer">
         <p className="text-[10px] font-bold uppercase text-blue-200/60 mb-1">Gold Coins left</p>
         <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <GoldCoinIcon className="h-4 w-4" />
            <span className="text-sm font-bold text-white ">{(userProfile?.wallet?.coins || 0).toLocaleString()}</span>
          </div>
       ...
       <div className="flex-1 bg-gradient-to-b from-[#1e3a8a] to-[#1e1b4b] rounded-2xl p-3 border-2 border-blue-400/20 shadow-xl">
         <p className="text-[10px] font-bold uppercase text-blue-200/60 mb-1">Today's profits</p>
         <div className="flex items-center gap-1.5">
          <GoldCoinIcon className="h-4 w-4" />
          <span className="text-sm font-bold text-yellow-400 ">{todayProfits.toLocaleString()}</span>
         </div>
       </div>
      </div>
    </main>
       ...
       <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar">
         {history.map((id, i) => (
          <div key={i} className="relative shrink-0 flex flex-col items-center">
           ...
           {i === 0 && <span className="absolute -top-1 -right-1 bg-green-500 text-[6px] px-1 rounded-full font-bold animate-pulse">New</span>}
          </div>
         ))}
       </div>
      </div>
