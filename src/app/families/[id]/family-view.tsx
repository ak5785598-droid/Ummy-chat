...
export function FamilyView({ familyId }: FamilyViewProps) {
  ...
  const membersQuery = useMemo(() => {
    ...
    return query(
      ...
      where('uid', 'in', family.members.slice(0, 10))
    );
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-32 animate-in fade-in duration-1000 relative text-slate-800">
         ...
         <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between">
            ...
            <div className="flex items-center gap-3 mb-2">
               ...
               {isMember ? (
                 ...
                 <Button onClick={handleJoin} className="rounded-full bg-purple-600 hover:bg-purple-700 text-white font-black uppercase text-xs h-12 px-8 shadow-xl shadow-purple-500/20">Join Family</Button>
               )}
            </div>
         </div>
      </section>
      ...
      <main className="px-6 space-y-8 mt-6 relative z-10">
         <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col items-center gap-2 text-center transition-all hover:border-purple-200">
         ...
         <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-3">
               {memberProfiles?.map((profile: any) => (
                  <div key={profile.uid} onClick={() => router.push(`/profile/${profile.uid}`)} className="flex items-center p-3 bg-white border border-slate-100 shadow-sm rounded-2xl hover:bg-slate-50 transition-all cursor-pointer group">
               ...
               {isMembersLoading && <div className="flex items-center justify-center py-10"><Loader className="animate-spin text-purple-600 h-5 w-5" /></div>}
            </div>
         </div>
      </main>
    </div>
