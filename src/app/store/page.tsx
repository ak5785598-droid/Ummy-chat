...
const CircleAvatar = ({ src, fallback, size = "md", rank, theme }: { src?: string; fallback: string; size?: "sm" | "md" | "lg"; rank?: number; theme?: LeaderboardThemeConfig | null }) => {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      {/* User Avatar - Pehle ye render hoga (neeche rahega) */}
      <div className={cn("relative z-10 flex items-center justify-center p-0.5 rounded-full border-2 border-white/20", sizes[size])}>
        <Avatar className="h-full w-full">
          <AvatarImage src={src} className="object-cover rounded-full" />
          <AvatarFallback className="bg-transparent text-white font-black rounded-full flex items-center justify-center">{fallback}</AvatarFallback>
        </Avatar>
      </div>

      {/* Frame - Ab ye Avatar ke upar aayega (z-20) aur mix-blend-mode: screen se black remove hoga */}
      {frame && (
        <div 
          className={cn("absolute z-20 pointer-events-none", frameSizes[size])}
          style={{ 
            mixBlendMode: 'screen',
            isolation: 'isolate'
          }}
