    ...
    <div className="relative z-10 space-y-8 p-6 max-w-4xl mx-auto pb-32">
     <header className="space-y-3 pt-10">
      <div className="inline-flex items-center gap-2 bg-white/40 backdrop-blur-xl px-4 py-1 rounded-full border border-white/50 shadow-sm">
        ...
        <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-green-800">Support Protocol</span>
      </div>
     ...
     <Card className="border-none bg-white shadow-[0_20px_50px_rgba(0,0,0,0.04)] rounded-3xl overflow-hidden relative group">
      <CardHeader className="bg-green-50/50 border-b border-green-100 p-8">
       <CardTitle className="font-sans text-2xl text-green-900 uppercase tracking-tight">Need Live Support?</CardTitle>
      </CardHeader>
      <CardContent className="space-y-8 p-8">
       <p className="text-gray-600 font-body text-xl leading-relaxed">
        Contact our official support team via email or look for members with the <span className="text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-lg border border-green-100">"Official"</span> badge in any frequency.
       </p>
       <div className="flex flex-col sm:flex-row items-center justify-between rounded-3xl border-2 border-green-100 bg-green-50/30 p-8 hover:bg-green-50 transition-all shadow-sm group/btn gap-6">
         <div className="flex items-center gap-6">
           ...
           <div>
             <h3 className="font-bold text-2xl uppercase tracking-tight text-green-900 ">Ummy Support</h3>
             <p className="text-[10px] text-green-600 font-bold uppercase tracking-[0.2em] mt-1">
               Response in 24 Hours Sync
             </p>
           </div>
         </div>
         <button className="w-full sm:w-auto bg-green-600 text-white font-bold uppercase tracking-wider text-base px-12 py-5 rounded-full shadow-2xl transition-all hover:scale-105 active:scale-95">
           Email Us
         </button>
       </div>
      </CardContent>
     ...
     <div className="space-y-6">
      <div className="flex items-center gap-4 ml-4">
        ...
        <h2 className="text-2xl font-bold uppercase tracking-tight text-green-900 ">
         FAQ Dimension
        </h2>
      </div>
      
      <Card className="border-none shadow-sm rounded-3xl bg-white/80 backdrop-blur-md overflow-hidden border border-white/50">
       <CardContent className="p-4">
        <Accordion type="single" collapsible className="w-full">
         {faqs.map((faq, index) => (
          <AccordionItem value={`item-${index}`} key={index} className="border-green-50/50 px-6 last:border-0">
           <AccordionTrigger className="text-left font-bold uppercase text-lg py-8 text-slate-800 hover:text-green-600 hover:no-underline transition-colors gap-4">
            {faq.question}
           </AccordionTrigger>
           <AccordionContent className="text-gray-500 font-body text-xl pb-8 leading-relaxed">
            {faq.answer}
           </AccordionContent>
          </AccordionItem>
         ))}
