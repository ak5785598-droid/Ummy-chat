  ...
  <AppLayout>
   <div className="space-y-8 max-w-5xl mx-auto p-4 animate-in fade-in duration-700 font-sans pb-32">
    <header className="flex flex-col gap-2 pt-safe">
     ...
     <p className="text-muted-foreground font-body text-lg px-1">Complete tribal duties to earn Gold and prestige.</p>
    </header>
    ...
    <Card className="border-none shadow-xl rounded-3xl overflow-hidden bg-white">
     ...
     <CardContent className="space-y-6">
      ...
      <Button 
       ...
       className={cn(
        "w-full h-16 rounded-3xl text-xl font-bold uppercase shadow-xl transition-all",
    ...
    <section className="space-y-4">
     <div className="flex items-center justify-between px-2">
      ...
      <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Resets in 12h</span>
     </div>
      ...
      {isTasksLoading ? (
       ...
       tasksWithStatus.map((task) => (
        <Card key={task.id} className={cn(
         "rounded-3xl border-none shadow-xl transition-all relative overflow-hidden group", 
         task.isCollected 
         ...
         <CardContent className="flex flex-col gap-4 relative z-20">
          <div className="flex items-center justify-between">
            ...
            {task.isCollected ? (
             <div className="flex items-center gap-1 text-green-500 text-[10px] font-bold uppercase bg-green-50 px-3 py-1 rounded-full border border-green-100">
             ...
             <Button 
              ...
              className="rounded-full px-8 font-bold uppercase text-[10px] h-10 bg-yellow-400 text-black hover:bg-yellow-500 shadow-lg shadow-yellow-900/20 active:scale-95 transition-all"
             >
              {isCollecting === task.id ? <Loader className="animate-spin h-4 w-4" /> : <GiftIcon className="h-4 w-4 mr-2" />}
              Collect
             </Button>
            ) : (
             <Button asChild size="sm" className="rounded-full px-8 font-bold uppercase text-[10px] h-10 bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-900/20 active:scale-95 transition-all">
          ...
          {user?.uid === CREATOR_ID && !task.isCompleted && !task.isCollected && (
           <button 
            ...
            className="text-[8px] font-bold uppercase tracking-wider text-slate-300 hover:text-slate-500 transition-colors"
           >
       ...
       <div className="col-span-full py-10 text-center opacity-20 ">No tasks currently assigned by tribal authority.</div>
      )}
     </div>
    </section>
   </div>
