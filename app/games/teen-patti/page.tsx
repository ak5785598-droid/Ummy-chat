           ...
           <div className="grid grid-cols-3 gap-2 px-4 h-44">
              {FACTIONS.map((f) => (
                <div key={f.id} className="flex flex-col items-center gap-2">
                   ...
                   <div className="text-center"><p className="text-[9px] font-bold text-white/60 uppercase">Pot:{(totalPots[f.id] || 0).toLocaleString()}</p><p className="text-[9px] font-black text-[#ffd700] uppercase italic">Me:{(myBets[f.id] || 0).toLocaleString()}</p></div>
                </div>
              ))}
           </div>
        ...
        <footer className="fixed bottom-0 left-0 right-0 p-4 pb-10 bg-gradient-to-t from-black via-black/90 to-transparent z-[120]">
           <div className="max-w-xl mx-auto flex items-center justify-between gap-4">
              <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center gap-2 pl-3 pr-4 py-2 shadow-2xl">
                <GoldCoinIcon className="h-5 w-5 text-[#ffd700]" /><span className="text-[15px] font-black text-[#ffd700] italic">{(userProfile?.wallet?.coins || 0).toLocaleString()}</span>
              </div>
