          ...
          <div className="flex-1 w-full min-w-0">
               ...
               <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8">
                  ...
                  <CardContent className="px-0 space-y-10">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="p-6 bg-emerald-50 rounded-3xl border-2 border-emerald-100 flex flex-col gap-1"><p className="text-[10px] font-black uppercase text-emerald-600 tracking-widest">Total Synchronized Users</p><div className="flex items-center gap-2 text-2xl font-black text-emerald-900 italic"><Users className="h-6 w-6" />{appStats.totalUsers.toLocaleString()}</div></div>
                     </div>
                  </CardContent>
               </Card>
            </TabsContent>
               ...
               <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8">
                  ...
                  <CardContent className="px-0 space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                        <div className="space-y-4">
                           <Input placeholder="Asset Name..." value={storeName} onChange={(e) => setStoreName(e.target.value)} className="h-14 rounded-2xl border-2 border-emerald-100" />
                           <div className="grid grid-cols-2 gap-2">
                              <Input type="number" placeholder="Price" value={storePrice} onChange={(e) => setStorePrice(e.target.value)} className="h-12 rounded-xl border-emerald-100" />
                              <Input type="number" placeholder="Days" value={storeDuration} onChange={(e) => setStoreDuration(e.target.value)} className="h-12 rounded-xl border-emerald-100" />
                           </div>
                           <Select value={storeCategory} onValueChange={(v: any) => setStoreCategory(v)}>
                              <SelectTrigger className="h-14 rounded-2xl border-emerald-100"><SelectValue /></SelectTrigger>
                        ...
                        <div className="flex flex-col items-center justify-center border-2 border-dashed border-emerald-200 rounded-3xl bg-white p-6 group">
                           <button onClick={() => storeFileInputRef.current?.click()} className="flex flex-col items-center gap-3">
                              {isUploadingStore ? <Loader className="animate-spin text-emerald-600" /> : <Upload className="h-8 w-8 text-emerald-400" />}
                              <span className="text-[10px] font-black uppercase text-emerald-600">Upload Visual</span>
                           </button>
               ...
               <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8">
                  ...
                  <CardContent className="px-0 space-y-6">
                     <div className="flex flex-col gap-4">
                        ...
                        <div className="flex gap-4"><Input placeholder="Enter member signature..." className="h-14 rounded-2xl border-2 border-emerald-100" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearchUsers()} /><Button onClick={handleSearchUsers} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white" disabled={isSearching}>{isSearching ? <Loader className="animate-spin" /> : 'Find'}</Button></div>
                     </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {foundUsers.map((u) => (
                          <div key={u.id} className="p-6 bg-slate-50 rounded-3xl border-2 border-slate-100 flex flex-col gap-4 shadow-sm">
                             ...
                             <div className="grid grid-cols-2 gap-2">
                                {AUTHORITY_ROLES.map(role => (<Button key={role.id} variant={u.tags?.includes(role.id) ? 'default' : 'outline'} size="sm" onClick={() => toggleUserRole(u.id, role.id, u.tags)} className={cn("h-10 text-[8px] font-black uppercase rounded-xl", u.tags?.includes(role.id) ? "bg-emerald-600" : "border-emerald-100 text-emerald-600 hover:bg-emerald-50")}>{role.label}</Button>))}
                             </div>
                          </div>
                        ))}
                     </div>
            ...
            <TabsContent value="id-ban" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  <div className="flex flex-col gap-4"><SearchToggle mode={banSearchMode} setMode={setBanSearchMode} /><div className="flex gap-4"><Input placeholder="Enter Target..." value={banSearchId} onChange={(e) => setBanSearchId(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(banSearchMode, banSearchId, setTargetUserForBan, setIsSearchingBan)} className="h-14 rounded-2xl border-2 border-emerald-100" /><Button onClick={() => handleGenericSearch(banSearchMode, banSearchId, setTargetUserForBan, setIsSearchingBan)} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white font-black uppercase italic" disabled={isSearchingBan}>Locate</Button></div></div>
                  {targetUserForBan && (<div className="mt-10 p-8 border-2 border-emerald-100 rounded-[2.5rem] space-y-8 animate-in slide-in-from-bottom-4 bg-emerald-50/20"><div className="flex items-center justify-between border-b border-emerald-100 pb-6"><div className="flex items-center gap-4"><Avatar className="h-16 w-16 border-2 border-white shadow-xl"><AvatarImage src={targetUserForBan.avatarUrl || undefined}/></Avatar><div><p className="font-black uppercase italic text-xl tracking-tighter text-slate-900">{targetUserForBan.username}</p></div></div><div>{targetUserForBan.banStatus?.isBanned ? (<Badge className="bg-red-600 text-white">BANNED</Badge>) : (<Badge className="bg-emerald-500 text-white">OK</Badge>)}</div></div>{!targetUserForBan.banStatus?.isBanned ? (
                    <div className="space-y-6">
                       <div className="grid grid-cols-4 gap-4">{[{ l: 'Days', v: banDays, s: setBanDays }, { l: 'Hrs', v: banHours, s: setBanHours }, { l: 'Min', v: banMinutes, s: setBanMinutes }, { l: 'Sec', v: banSeconds, s: setBanSeconds }].map(i => (<div key={i.l} className="space-y-1"><p className="text-[8px] font-black uppercase text-gray-400 text-center">{i.l}</p><Input value={i.v} onChange={(e) => i.s(e.target.value.replace(/\D/g, ''))} className="h-12 rounded-xl text-center border-emerald-100" /></div>))}</div>
                  ...
                  <CardContent className="px-0 space-y-8"><div className="grid grid-cols-1 gap-8">{(bannerConfig?.slides || DEFAULT_SLIDES).map((slide: any, idx: number) => (
                    <div key={idx} className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 flex flex-col md:flex-row gap-8"><div className="w-72 h-40 relative rounded-2xl overflow-hidden bg-slate-200">{slide.imageUrl && (<Image src={slide.imageUrl} alt="Banner" fill className="object-cover" unoptimized />)}<button onClick={() => bannerFileInputRefs.current[idx]?.click()} className="absolute bottom-3 right-3 bg-white p-2 rounded-full shadow-lg text-emerald-600"><Camera className="h-4 w-4" /></button><input type="file" ref={el => { bannerFileInputRefs.current[idx] = el; }} className="hidden" onChange={(e) => e.target.files?.[0] && handleBannerImageUpload(idx, e.target.files[0])} /></div><div className="flex-1 space-y-4">
                      <div className="space-y-1">
                        ...
                        <Input value={slide.title} onChange={(e) => handleUpdateBannerMeta(idx, 'title', e.target.value)} className="h-12 rounded-xl border-emerald-100" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Subtitle</Label>
                        <Input value={slide.subtitle} onChange={(e) => handleUpdateBannerMeta(idx, 'subtitle', e.target.value)} className="h-12 rounded-xl border-emerald-100" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Link (URL)</Label>
                        <Input placeholder="https://..." value={slide.link || ''} onChange={(e) => handleUpdateBannerMeta(idx, 'link', e.target.value)} className="h-12 rounded-xl border-emerald-100" />
                      </div>
            ...
            <TabsContent value="broadcaster" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8">
                  ...
                  <CardContent className="px-0 space-y-6"><div className="space-y-4"><Input value={broadcastTitle} onChange={(e) => setBroadcastTitle(e.target.value)} className="h-14 rounded-2xl border-emerald-100" /><Textarea placeholder="Type broadcast..." value={broadcastContent} onChange={(e) => setBroadcastContent(e.target.value)} className="h-40 rounded-3xl border-emerald-100" /></div><Button onClick={handleSystemBroadcast} disabled={isBroadcasting || !broadcastContent.trim()} className="w-full h-16 rounded-[1.5rem] bg-emerald-600 text-white">Synchronize Broadcast</Button></CardContent>
               </Card>
            </TabsContent>
            ...
            <TabsContent value="direct-messenger" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  <div className="flex flex-col gap-4"><SearchToggle mode={dmSearchMode} setMode={setDmSearchMode} /><div className="flex gap-4"><Input placeholder="Recipient..." value={dmSearchId} onChange={(e) => setDmSearchId(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(dmSearchMode, dmSearchId, setTargetUserForDm, setIsSearchingDm)} className="h-14 rounded-2xl border-emerald-100" /><Button onClick={() => handleGenericSearch(dmSearchMode, dmSearchId, setTargetUserForDm, setIsSearchingDm)} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white" disabled={isSearchingDm}>Find</Button></div></div>
                  {targetUserForDm && (<div className="mt-10 p-8 border-2 border-emerald-100 rounded-[2.5rem] space-y-8"><div className="flex items-center gap-4"><Avatar className="h-16 w-16"><AvatarImage src={targetUserForDm.avatarUrl || undefined}/></Avatar><p className="font-black uppercase text-xl">{targetUserForDm.username}</p></div><div className="space-y-4"><Input value={dmTitle} onChange={(e) => setDmTitle(e.target.value)} className="h-14 rounded-2xl border-emerald-100" /><Textarea placeholder="Private msg..." value={dmContent} onChange={(e) => setDmContent(e.target.value)} className="h-40 rounded-3xl border-emerald-100" /></div><Button onClick={handleDirectMessage} disabled={isSendingDm || !dmContent.trim()} className="w-full h-16 rounded-[1.5rem] bg-emerald-600 text-white">Send Sync</Button></div>)}
               </Card>
            </TabsContent>
            ...
            <TabsContent value="tags" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  <div className="flex flex-col gap-4"><SearchToggle mode={tagSearchMode} setMode={setTagSearchMode} /><div className="flex gap-4"><Input placeholder="Target..." value={tagSearchId} onChange={(e) => setTagSearchId(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(tagSearchMode, tagSearchId, setTargetUserForTags, setIsSearchingTag)} className="h-14 rounded-2xl border-emerald-100" /><Button onClick={() => handleGenericSearch(tagSearchMode, tagSearchId, setTargetUserForTags, setIsSearchingTag)} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white" disabled={isSearchingTag}>Locate</Button></div></div>
                  {targetUserForTags && (<div className="mt-10 p-6 border-2 border-emerald-100 rounded-[2rem] flex flex-col gap-8"><div className="flex items-center gap-4"><Avatar className="h-16 w-16"><AvatarImage src={targetUserForTags.avatarUrl || undefined}/></Avatar><p className="font-black text-xl">{targetUserForTags.username}</p></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-4">{ELITE_TAGS.map(tag => (<Button key={tag.id} variant={targetUserForTags.tags?.includes(tag.id) ? 'default' : 'outline'} className={cn("h-12 rounded-xl", targetUserForTags.tags?.includes(tag.id) ? "bg-emerald-600" : "border-emerald-100 text-emerald-600")} onClick={() => toggleUserRole(targetUserForTags.id, tag.id, targetUserForTags.tags)}>{tag.label}</Button>))}</div><Button variant="ghost" onClick={() => handleRemoveAllTags(targetUserForTags.id)} className="text-red-500 text-[10px]">Purge All Tags</Button></div>)}
               </Card>
            </TabsContent>
            ...
            <TabsContent value="special-id" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  <div className="flex flex-col gap-4"><SearchToggle mode={specialIdSearchMode} setMode={setSpecialIdSearchMode} /><div className="flex gap-4"><Input placeholder="Enter Identity..." value={idSearchInput} onChange={(e) => setIdSearchInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(specialIdSearchMode, idSearchInput, setTargetUserForId, setIsSearching)} className="h-14 rounded-2xl border-emerald-100" /><Button onClick={() => handleGenericSearch(specialIdSearchMode, idSearchInput, setTargetUserForId, setIsSearching)} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white">Find</Button></div></div>
                  {targetUserForId && (<div className="mt-10 p-6 border-2 border-emerald-100 rounded-[2rem] space-y-8"><div className="flex items-center gap-4"><Avatar className="h-16 w-16"><AvatarImage src={targetUserForId.avatarUrl || undefined}/></Avatar><p className="font-black text-xl">{targetUserForId.username}</p></div><div className="space-y-6"><div className="flex gap-2"><Input placeholder="New ID..." value={newIdInput} onChange={(e) => setNewIdInput(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl border-emerald-100" /><Button onClick={handleUpdateId} disabled={!newIdInput || isSavingId} className="h-14 px-10 bg-emerald-600 text-white">Sync</Button><Button onClick={() => handleRemoveId()} variant="outline" className="h-14 border-red-500 text-red-500"><Trash2 /></Button></div></div></div>)}
               </Card>
            </TabsContent>
            ...
            <TabsContent value="rewards" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  <div className="flex flex-col gap-4"><SearchToggle mode={rewardSearchMode} setMode={setRewardSearchMode} /><div className="flex gap-4"><Input placeholder="Recipient..." value={rewardSearchId} onChange={(e) => setRewardSearchId(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGenericSearch(rewardSearchMode, rewardSearchId, setTargetUserForRewards, setIsSearchingRewards)} className="h-14 rounded-2xl border-emerald-100" /><Button onClick={() => handleGenericSearch(rewardSearchMode, rewardSearchId, setTargetUserForRewards, setIsSearchingRewards)} className="h-14 px-8 rounded-2xl bg-emerald-600 text-white">Find</Button></div></div>
                  {targetUserForRewards && (<div className="mt-10 p-8 border-2 border-emerald-100 rounded-[2.5rem] space-y-10"><div className="flex items-center gap-4"><Avatar className="h-16 w-16"><AvatarImage src={targetUserForRewards.avatarUrl || undefined}/></Avatar><p className="font-black text-xl">{targetUserForRewards.username}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10"><div className="space-y-4"><h4>Coin Dispatch</h4><div className="flex gap-2"><Input value={coinDispatchAmount} onChange={(e) => setCoinDispatchAmount(e.target.value.replace(/\D/g, ''))} className="h-14 border-emerald-100" /><Button onClick={handleDispatchCoins} className="h-14 bg-emerald-600 text-white"><Send /></Button></div></div><div className="space-y-4"><h4>Elite Assets</h4><div className="flex flex-wrap gap-2">{DISPATCH_ASSETS.frames.map(frame => (<Button key={frame.id} variant="outline" size="sm" onClick={() => handleDispatchItem(frame.id, 'ownedItems')} className="text-[8px] font-black uppercase rounded-xl border-emerald-600 text-emerald-600 hover:bg-emerald-50">{frame.name}</Button>))}</div></div></div></div>)}
               </Card>
            </TabsContent>
