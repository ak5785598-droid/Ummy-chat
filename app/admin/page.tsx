          ...
          <div className="flex-1 w-full min-w-0">
            ...
            <TabsContent value="special-id" className="m-0 space-y-6">
               <Card className="rounded-[2.5rem] border-none shadow-xl p-8 bg-white">
                  ...
                  {targetUserForId && (<div className="mt-10 p-6 border-2 rounded-[2rem] space-y-8"><div className="flex items-center gap-4"><Avatar className="h-16 w-16"><AvatarImage src={targetUserForId.avatarUrl || undefined}/></Avatar><p className="font-black text-xl">{targetUserForId.username}</p></div><div className="space-y-6"><div className="flex gap-2"><Input placeholder="New ID..." value={newIdInput} onChange={(e) => setNewIdInput(e.target.value.replace(/\D/g, ''))} className="h-14 rounded-2xl" /><Button onClick={handleUpdateId} disabled={!newIdInput || isSavingId} className="h-14 px-10">Sync</Button><Button onClick={() => handleRemoveId()} variant="outline" className="h-14"><Trash2 /></Button></div></div></div>)}
               </Card>
            </TabsContent>
